...
public class PLCode extends DbsmTest {
  ...
  @Test
  public void thereShouldNoBeTriggerRecreationIfNotModified() {
    resetDB();
  ...
  @Test
  public void thereShouldNoBeFunctionRecreationIfNotModified() {
    resetDB();
    String dbModelPath = "functions/SIMPLE_FUNCTION.xml";
    updateDatabase(dbModelPath);

    List<String> updateStatements = sqlStatmentsForUpdate(dbModelPath);

    assertThat("changes between updated db and target db", updateStatements,
        not(hasItem(containsString("DROP FUNCTION"))));
  }
}
