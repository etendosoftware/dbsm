	...
	public void doExecute() {
		...
		try {
			...
			for (String dataSetCode : datasets) {
				...
				{
				    if(module==null || module.equals("%") || util.isIncludedInExportList(util.getActiveModule(i)))
				    {
				      ...
				      if (util.getActiveModule(i).name.equalsIgnoreCase("CORE")) {
				        ...
				        // First we delete all .xml files in the directory
    						...
    						for (DataSetTable table : tableList) {
    							_log.info("Exporting table: "+ table.getTable().getTableName()+ " to Core");
    							OutputStream out = new FileOutputStream(new File(getOutput(), table.getTable().getTableName().toUpperCase()+ ".xml"));
    							dbdio.writeDataForTableToXML(originaldb,datasetService, dataSetCode, table, out,getEncoding(),util.getActiveModule(i).idMod);
    							out.flush();
    						}
				      } else {
    						if (dataSetCode.equals("AD")) {
    							File path = new File(moduledir, util.getActiveModule(i).dir+ "/src-db/database/sourcedata/");
    							_log.info("Path: " + path);
    							path.mkdirs();
    							if (datasetI == 0) {
    								File[] filestodelete = DatabaseIO.readFileArray(path);
    								for (File filedelete : filestodelete) {
    									filedelete.delete();
    								}
    							}
    							for (DataSetTable table : tableList) {
    								_log.info("Exporting table: "+ table.getTable().getTableName()+ " to module "+util.getActiveModule(i).name);
    								File tableFile = new File(path, table.getTable().getTableName().toUpperCase()+ ".xml");
    								OutputStream out = new FileOutputStream(tableFile);
    								boolean b = dbdio.writeDataForTableToXML(originaldb, datasetService,dataSetCode, table, out, getEncoding(),util.getActiveModule(i).idMod);
    								if (!b) tableFile.delete();
    								out.flush();
    							}
    						}
				      }
