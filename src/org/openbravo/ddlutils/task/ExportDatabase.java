        ...
        if (module == null || module.equals("%")) {
            ...
            for (int i = 0; i < util.getActiveModuleCount(); i++) {
                ...
                if (util.getActiveModule(i).name.equalsIgnoreCase("CORE")) {
                    ...
                    File path = new File(moduledir, util.getActiveModule(i).dir
            ...
            for (int i = 0; i < util.getActiveModuleCount(); i++) {
                ...
                if (util.isIncludedInExportList(row)) {
                    ...
                    if (row == null)
                        throw new BuildException("Module " + row.name
                                + " not found in AD_MODULE table.");
                    if (row.prefixes.size() == 0) {
                        _log
                                .info("Module doesn't have dbprefix. We will not export structure for it.");
                        return;
                    }
                    if (row.isInDevelopment != null
                        ...
                        DatabaseIO io = new DatabaseIO();
                        File path = new File(moduledir, row.dir
                                + "/src-db/database/model/");
                        _log.info("Path: " + path);
                        io.writeToDir(db, path);
                    } else {
                        _log
                                .info("Module is not in development. Check that it is, before trying to export it.");
                    }
                }
            }
        }
