 ...
 * @author adrian
 */
public class AlterDatabaseDataMP extends BaseDalInitializingTask {
    ...
    try {
      ...
      if (dataComparatorDS.getChanges().size() > 0) {
        ...
        try {
          ...
          getLog().error("generate.entities process failed: " + e.getMessage(), e);
        }
        // stop here for now
        if (!secondPass) {
          getLog().info("Entities generated, restarting update database to use new classes");
          return;
        }
