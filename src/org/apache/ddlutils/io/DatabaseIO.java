...
public class DatabaseIO {
		...
		if (f.isDirectory()) {
			...
			File[] directoryfiles = f.listFiles(new FileFilter() {
				public boolean accept(File pathname) {
					return !pathname.isHidden() && (pathname.isDirectory() || (pathname.isFile() && pathname.getName().endsWith(".xml")));
				}
			});
	...
	public Vector<Change> readChanges(File file)
	{
		try{
			...
			{
				Change change = (Change)reader.parse(new ByteArrayInputStream((line).getBytes()));
				changes.add(change);
			}
			return changes;
