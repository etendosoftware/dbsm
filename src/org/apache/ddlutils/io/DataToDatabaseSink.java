...
/**
 * Data sink that directly inserts the beans into the database. If configured, it will make sure
 * that the beans are inserted in the correct order according to the foreignkeys. Note that this
 * will only work if there are no circles.
 * 
...
public class DataToDatabaseSink implements DataSink {
  ...
  /**
   * Whether to delay the insertion of beans so that the beans referenced by it via foreignkeys, are
   * already inserted into the database.
   */
  ...
  /**
   * Contains the tables that have a self-referencing foreign key to a (partially) identity primary
   * key.
   */
  ...
  /**
   * Determines whether this sink halts when an error happens during the insertion of a bean into
   * the database. Default is <code>true</code>.
   * 
  ...
  /**
   * Specifies whether this sink halts when an error happens during the insertion of a bean into the
   * database.
   * 
   * @param haltOnErrors
   *          <code>true</code> if the sink shall stop when an error occurred
   */
   ...
   * end of the insertion process (method {@link #end()}).
   * 
   * @param ensureFkOrder
  ...
  /**
   * Specifies whether batch mode is used for inserting the beans. Note that this requires that the
   * primary key values are not defined by the database.
   * 
   * @param useBatchMode
  ...
  /**
   * Directly inserts the given bean into the database with delete old data from table. before you
   * are recommended disable all trigger and FK constraint
   * 
   * @param table
   *          The table of the bean
   * @param bean
  ...
  /**
   * Returns the name of the given foreign key. If it has no name, then a temporary one is generated
   * from the names of the relevant tables and columns.
   * 
   * @param owningTable
  ...
  /**
   * Builds an identity object from the primary keys of the specified table using the column values
  ...
  /**
   * Builds an identity object for the specified foreign key using the foreignkey column values of
   * the supplied bean.
   * 
   * @param owningTable
  ...
  /**
   * Updates the values of the columns constituting the indicated foreign key with the values of the
   * given identity.
   * 
   * @param bean
