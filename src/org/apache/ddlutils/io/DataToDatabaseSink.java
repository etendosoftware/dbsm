...
/**
 * Data sink that directly inserts the beans into the database. If configured,
 * it will make sure that the beans are inserted in the correct order according
 * to the foreignkeys. Note that this will only work if there are no circles.
 * 
...
public class DataToDatabaseSink implements DataSink {
    ...
    /**
     * Whether to stop when an error has occurred while inserting a bean into
     * the database.
     */
    private boolean _haltOnErrors = true;
    /**
     * Whether to delay the insertion of beans so that the beans referenced by
     * it via foreignkeys, are already inserted into the database.
     */
    ...
    /**
     * Contains the tables that have a self-referencing foreign key to a
     * (partially) identity primary key.
     */
    private HashSet _tablesWithSelfIdentityReference = new HashSet();
    /**
     * Contains the tables that have a self-referencing foreign key that is
     * required.
     */
        ...
        for (int tableIdx = 0; tableIdx < model.getTableCount(); tableIdx++) {
            ...
            if (selfRefFk != null) {
                ...
                for (int idx = 0; idx < selfRefFk.getReferenceCount(); idx++) {
                    if (selfRefFk.getReference(idx).getLocalColumn()
                            .isRequired()) {
    ...
    /**
     * Determines whether this sink halts when an error happens during the
     * insertion of a bean into the database. Default is <code>true</code>.
     * 
    ...
    /**
     * Specifies whether this sink halts when an error happens during the
     * insertion of a bean into the database.
     * 
     * @param haltOnErrors
     *            <code>true</code> if the sink shall stop when an error
     *            occurred
     */
     ...
     * beans referenced by it via foreignkeys are already inserted into the
     * database.<br/>
     * Note that you should careful with setting <code>haltOnErrors</code> to
     * false as this might result in beans not inserted at all. The sink will
     * then throw an appropriate exception at the end of the insertion process
     * (method {@link #end()}).
     * 
     * @param ensureFkOrder
     ...
     * this requires that the primary key values are not defined by the
     * database.
     * 
     * @param useBatchMode
        ...
        if (!_waitingObjects.isEmpty()) {
            if (_log.isDebugEnabled()) {
                for (Iterator it = _waitingObjects.iterator(); it.hasNext();) {
                    ...
                    Table table = _model.getDynaClassFor(obj.getObject())
                            .getTable();
                    Identity objId = buildIdentityFromPKs(table, obj
                            .getObject());
    ...
    public void start() throws DataSinkException {
        ...
        if (_databasefilter != null) {
            ...
            _platform.deleteDataFromTable(_connection, _model, tablenames,
                    tableFilters, _haltOnErrors);
    ...
    public void addBean(DynaBean bean) throws DataSinkException {
        ...
        if (_ensureFkOrder && (table.getForeignKeyCount() > 0)) {
            ...
            for (int idx = 0; idx < table.getForeignKeyCount(); idx++) {
                ...
                if ((fkIdentity != null) && !fkIdentity.equals(origIdentity)) {
                    Identity processedIdentity = (Identity) _identityMap
                            .get(fkIdentity);

                    if (processedIdentity != null) {
                        updateFKColumns(bean, fkIdentity.getForeignKeyName(),
                                processedIdentity);
                    } else {
        ...
        if (_ensureFkOrder && _fkTables.contains(table)) {
            ...
            // we're doing multiple passes so that we can insert as much objects
            // in
            // one go as possible
            ...
            while (!identitiesToCheck.isEmpty() && !_waitingObjects.isEmpty()) {
                ...
                Identity curNewIdentity = (Identity) _identityMap
                        .get(curIdentity);

                identitiesToCheck.remove(0);
                finishedObjs.clear();
                for (Iterator waitingObjIt = _waitingObjects.iterator(); waitingObjIt
                        .hasNext();) {
                    WaitingObject waitingObj = (WaitingObject) waitingObjIt
                            .next();
                    Identity fkIdentity = waitingObj
                            .removePendingFK(curIdentity);

                    if (fkIdentity != null) {
                        updateFKColumns(waitingObj.getObject(), fkIdentity
                                .getForeignKeyName(), curNewIdentity);
                    }
                    if (!waitingObj.hasPendingFKs()) {
                        waitingObjIt.remove();
                ...
                for (Iterator finishedObjIt = finishedObjs.iterator(); finishedObjIt
                        .hasNext();) {
                    DynaBean finishedObj = (DynaBean) finishedObjIt.next();
                    Table tableForObj = _model.getDynaClassFor(finishedObj)
                            .getTable();
                    Identity objIdentity = buildIdentityFromPKs(tableForObj,
                            finishedObj);

                    insertBeanIntoDatabase(tableForObj, finishedObj);

                    Identity newObjIdentity = buildIdentityFromPKs(tableForObj,
                            finishedObj);
     ...
     *            The bean
     */
    private void insertBeanIntoDatabase(Table table, DynaBean bean)
        ...
        if (_useBatchMode) {
            ...
            upsertSingleBeanIntoDatabase(table, bean); // this method ignored FK
                                                       // because it must be
                                                       // disabled originally
        }
    }
    ...
    private void purgeBatchQueue() throws DataSinkException {
        if (!_batchQueue.isEmpty()) {
            try {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("Inserted " + _batchQueue.size()
                            + " rows in batch mode ");
                }
            } catch (Exception ex) {
                if (_haltOnErrors) {
    ...
    private void insertSingleBeanIntoDatabase(Table table, DynaBean bean)
        ...
        try {
            ...
            if (!_platform.isIdentityOverrideOn()
                ...
                // in case of a self-reference (fk points to the very row that
                // we're inserting)
                // and (at least) one of the pk columns is an identity column,
                // we first need
                // to insert the row with the fk columns set to null
                Identity pkIdentity = buildIdentityFromPKs(table, bean);
                Identity fkIdentity = buildIdentityFromFK(table, selfRefFk,
                        bean);
            ...
            if (needTwoStepInsert) {
                // we first insert the bean without the fk, then in the second
                // step we update the bean
                // with the row with the identity pk values
                ArrayList fkValues = new ArrayList();

                for (int idx = 0; idx < selfRefFk.getReferenceCount(); idx++) {
                    String columnName = selfRefFk.getReference(idx)
                            .getLocalColumnName();
                ...
                for (int idx = 0; idx < selfRefFk.getReferenceCount(); idx++) {
                    bean.set(selfRefFk.getReference(idx).getLocalColumnName(),
                            fkValues.get(idx));
                }
            ...
            if (_haltOnErrors) {
                ...
                _log.warn("Exception while inserting a row into the database",
                        ex);
            }
        }
    }
    ...
    /**
     * Directly inserts the given bean into the database with delete old data
     * from table. before you are recommended disable all trigger and FK
     * constraint
     * 
     * @param table
     *            The table of the bean
     ...
     *            The bean
     */
    private void upsertSingleBeanIntoDatabase(Table table, DynaBean bean)
            throws DataSinkException {
     ...
     * temporary one is generated from the names of the relevant tables and
     * columns.
     * 
     * @param owningTable
     *            The table owning the fk
     * @param fk
     *            The foreign key
     * @return The name
     */
    ...
    /**
     * Builds an identity object from the primary keys of the specified table
     * using the column values of the supplied bean.
     * 
     * @param table
     *            The table
     * @param bean
    ...
    private Identity buildIdentityFromPKs(Table table, DynaBean bean) {
        ...
        for (int idx = 0; idx < pkColumns.length; idx++) {
            identity.setColumnValue(pkColumns[idx].getName(), bean
                    .get(pkColumns[idx].getName()));
        }
        return identity;
    }

    /**
     * Builds an identity object for the specified foreign key using the
     * foreignkey column values of the supplied bean.
     * 
     * @param owningTable
     ...
     * @return The identity
     */
    private Identity buildIdentityFromFK(Table owningTable, ForeignKey fk,
            DynaBean bean) {
        Identity identity = new Identity(fk.getForeignTable(), getFKName(
                owningTable, fk));
    ...
    /**
     * Updates the values of the columns constituting the indicated foreign key
     * with the values of the given identity.
     * 
     * @param bean
     *            The bean whose columns shall be updated
     * @param fkName
    ...
    private void updateFKColumns(DynaBean bean, String fkName, Identity identity) {
        ...
        for (int idx = 0; idx < sourceTable.getForeignKeyCount(); idx++) {
            ...
            if (curFk.getForeignTableName().equalsIgnoreCase(
                    targetTable.getName())) {
        ...
        if (fk != null) {
            for (int idx = 0; idx < fk.getReferenceCount(); idx++) {
                ...
                bean.set(sourceColumn.getName(), identity
                        .getColumnValue(targetColumn.getName()));
            }
        }
    }
}
