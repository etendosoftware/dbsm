...
/**
 * A platform encapsulates the database-related functionality such as performing
 * queries and manipulations. It also contains an sql builder that is specific
 * to this platform.
 * 
...
public interface Platform {
     ...
     *            <code>true</code> if identity override is enabled (the
     *            default)
     */
    ...
    /**
     * Closes the given JDBC connection (returns it back to the pool if the
     * datasource is poolable).
     * 
     * @param connection
     *            The connection
     */
    ...
    /**
     * Executes a series of sql statements which must be seperated by the
     * delimiter configured as {@link PlatformInfo#getSqlCommandDelimiter()} of
     * the info object of this platform.
     * 
     * @param sql
     ...
     * the info object of this platform.
     * 
     * TODO: consider outputting a collection of String or some kind of
     * statement object from the SqlBuilder instead of having to parse strings
     * here
     * 
     * @param connection
     ...
     * connections. Note that this does not change the database structure or
     * data in it in any way.
     */
    ...
    /**
     * Creates the database specified by the given parameters. Please note that
     * this method does not use a data source set via
     * {@link #setDataSource(DataSource)} because it is not possible to retrieve
     * the connection information from it without establishing a connection.<br/>
    ...
    public String getCreateTablesSqlScript(Database model,
            boolean dropTablesFirst, boolean continueOnError);
     ...
     * 
     * @param catalog
     *            The catalog in the existing database to read (can be a
     *            pattern); use <code>null</code> for the platform-specific
     *            default value
     * @param schema
     *            The schema in the existing database to read (can be a
     *            pattern); use <code>null</code> for the platform-specific
     *            default value
     * @param tableTypes
    ...
    public void alterTables(String catalog, String schema, String[] tableTypes,
            Database desiredDb, boolean continueOnError)
            throws DatabaseOperationException;
    ...
    public String getAlterTablesSql(String catalog, String schema,
            String[] tableTypes, Database desiredDb)
            throws DatabaseOperationException;
     ...
     */
    public void alterTables(String catalog, String schema, String[] tableTypes,
            Database desiredDb, CreationParameters params,
            boolean continueOnError) throws DatabaseOperationException;
    ...
    public void alterTables(Connection connection, Database currentModel,
            Database desiredModel, boolean continueOnError)
            throws DatabaseOperationException;

    public void alterTables(Database currentModel, Database desiredModel,
            boolean continueOnError, List changes)
            throws DatabaseOperationException;

    public void alterTables(Database currentModel, Database desiredModel,
            boolean continueOnError) throws DatabaseOperationException;

    public void alterData(Connection connection, Database model,
            Vector<Change> changes) throws DatabaseOperationException;

    /**
     * Executes a small postscript to correct null constraints in new not-null
     ...
     * @throws DatabaseOperationException
     */
    public void alterTablesPostScript(Database currentModel,
            Database desiredModel, boolean continueOnError)
            throws DatabaseOperationException;
    ...
    public void alterTables(Connection connection, String catalog,
            String schema, String[] tableTypes, Database desiredDb,
            boolean continueOnError) throws DatabaseOperationException;
    ...
    /**
     * Queries for a list of dyna beans representing rows of the given query. In
     * contrast to the {@link #query(Database, String, Collection)} method all
     ...
     * contrast to the {@link #query(Database, String)} method all beans will be
     * materialized and the connection will be closed before returning the
     * beans. Also, the two int parameters specify which rows of the result set
     * to use. If there are more rows than desired, they will be ignored (and
    ...
    /**
     * Queries for a list of dyna beans representing rows of the given query. In
     * contrast to the {@link #query(Database, String, Table[])} method all
     * beans will be materialized and the connection will be closed before
     * returning the beans. Also, the two int parameters specify which rows of
     * the result set to use. If there are more rows than desired, they will be
     * ignored (and not read from the database).
    ...
    /**
     * Queries for a list of dyna beans representing rows of the given query. In
     * contrast to the {@link #query(Database, String, Collection, Table[])}
     ...
     * primary key otherwise the bean is updated in the database.
     * 
     * @param model
     *            The database model to use
     * @param dynaBean
    ...
    /**
     * Inserts the given DynaBean in the database, assuming the primary key
     ...
     * then the bean will also be updated with the column value generated by the
    ...
    /**
     * Inserts the given beans in the database, assuming the primary key values
     * are specified. Note that a batch insert is used for subsequent beans of
     * the same type. Also the properties for the primary keys are not updated
     * in the beans. Hence you should not use this method when the primary key
     * values are defined by the database (via a sequence or identity
     * constraint).
     * 
     * @param model
     *            The database model to use
     ...
     * updated in the beans. Hence you should not use this method when the
     * primary key values are defined by the database (via a sequence or
    ...
    /**
     * Updates the given bean in the database, assuming the primary key values
     * are specified.
     * 
     * @param model
     *            The database model to use
    ...
    /**
     * Updates or if not exists inserts the bean. If one of the columns is an
     * auto-incremented column, then the bean will also be updated with the
     * column value generated by the database. Note that the connection will not
    ...
    public void updateinsert(Connection connection, Database model,
    ...
    /**
     * Deletes the given bean from the database, assuming the primary key values
     * are specified.
     * 
     * @param model
     *            The database model to use
    ...
    public Database loadModelFromDatabase(ExcludeFilter filter)
            throws DatabaseOperationException;

    public Database loadModelFromDatabase(ExcludeFilter filter, String prefix,
            boolean loadCompleteTables, String moduleId)
            throws DatabaseOperationException;

    public Database loadModelFromDatabase(ExcludeFilter filter,
            String datasetName) throws DatabaseOperationException;

    /**
     * Reads the database model from the live database as specified by the data
     * source set for this platform.
     * 
     * @param name
     ...
     *            <code>null</code> itself though
     * @param catalog
     *            The catalog to access in the database; use <code>null</code>
     *            for the default value
     * @param schema
     *            The schema to access in the database; use <code>null</code>
     *            for the default value
     * @param tableTypes
     ...
     *            occurred
     */
    public void disableAllTriggers(Connection connection, Database model,
            boolean continueOnError) throws DatabaseOperationException;
     ...
     *            occurred
     */
    public void enableAllTriggers(Connection connection, Database model,
            boolean continueOnError) throws DatabaseOperationException;
     ...
     *            occurred
     */
    public void disableAllFK(Connection connection, Database model,
            boolean continueOnError) throws DatabaseOperationException;
     ...
     *            occurred
     */
    public void enableAllFK(Connection connection, Database model,
            boolean continueOnError) throws DatabaseOperationException;
     ...
     *            occurred
     */
    public void deleteDataFromTable(Connection connection, Database model,
            Table table, boolean continueOnError)
            throws DatabaseOperationException;

    public void deleteDataFromTable(Connection connection, Database model,
            String table, String sqlfilter, boolean continueOnError);

    public void deleteDataFromTable(Connection connection, Database model,
            String[] tables, String[] sqlfilters, boolean continueOnError);

    public ModelBasedResultSetIterator createResultSetIterator(Database model,
            ResultSet resultSet, Table[] queryHints);

    public void deleteInvalidConstraintRows(Database model,
            boolean continueOnError);

    public void deleteInvalidConstraintRows(Connection connection,
