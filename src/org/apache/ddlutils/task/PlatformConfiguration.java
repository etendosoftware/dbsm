...
public class PlatformConfiguration {
    ...
    /**
     * Specifies whether a table's foreign keys read from a live database shall
    ...
    /**
     * Specifies whether the database shall be shut down after the task has
     * finished.
     * 
    ...
    public Platform getPlatform() throws BuildException {
        ...
        if (_databaseType == null) {
            ...
            if (_databaseType == null) {
                _databaseType = new PlatformUtils().determineDatabaseType(
                        _dataSource.getDriverClassName(), _dataSource.getUrl());
            }
            if (_databaseType == null) {
                _databaseType = new PlatformUtils()
                        .determineDatabaseType(_dataSource);
            }
        }
        try {
            platform = PlatformFactory.createNewPlatformInstance(_databaseType);
        } catch (Exception ex) {
            throw new BuildException("Database type " + _databaseType
                    + " is not supported.", ex);
        }
        if (platform == null) {
            throw new BuildException("Database type " + _databaseType
