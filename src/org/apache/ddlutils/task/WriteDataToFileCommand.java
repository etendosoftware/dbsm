...
/**
 * Reads the data currently in the table in the live database (as specified by
 * the enclosing task), and writes it as XML to a file.
 * 
...
public class WriteDataToFileCommand extends ConvertingDatabaseCommand {
    ...
    public void execute(DatabaseTaskBase task, Database model)
        ...
        try {
            ...
            getDataIO().setSchemaPattern(
                    task.getPlatformConfiguration().getSchemaPattern());
