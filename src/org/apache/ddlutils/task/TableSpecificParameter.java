...
/**
 * Specifies a parameter for the creation of the tables. These are usually
 * platform specific. Note that parameters are only applied when creating new
 * tables, not when altering existing ones. Note also that if no table name is
 * specified, the parameter is used for all created tables.
 * 
...
public class TableSpecificParameter extends Parameter {
    ...
    /**
     * Specifies the comma-separated list of table names in whose creation this
     * parameter shall be used. For every table not in this list, the parameter
     * is ignored.
     * 
     * @param tableList
     *            The tables
     * @ant.not-required Use this or the <code>table</code> parameter. If
     *                   neither is specified, the parameter is applied in the
    ...
    /**
     * Specifies the name of the table in whose creation this parameter shall be
     * applied.
     * 
     * @param tableName
     *            The table
     * @ant.not-required Use this or the <code>tables</code> parameter. If
     *                   neither is specified, the parameter is applied in the
     *                   creation of all tables.
     */
    ...
    public boolean isForTable(Table table, boolean caseSensitive) {
        ...
        for (Iterator it = _tables.iterator(); it.hasNext();) {
            ...
            if ((caseSensitive && tableName.equals(table.getName()))
                    || (!caseSensitive && tableName.equalsIgnoreCase(table
                            .getName()))) {
                return true;
            }
        }
