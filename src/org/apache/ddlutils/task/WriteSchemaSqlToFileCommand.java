...
/**
 * Parses the schema XML files specified in the enclosing task, and writes the
 * SQL statements necessary to create this schema in the database, to a file.
 * Note that this SQL is database specific and hence this subtask requires that
 * for the enclosing task, either a data source is specified (via the
 * <code>database</code> sub element) or the <code>databaseType</code> attribute
 * is used to specify the database type.
 * 
...
public class WriteSchemaSqlToFileCommand extends
    ...
    /**
     * Determines whether to alter the database if it already exists, or re-set
     * it.
     * 
    ...
    /**
     * Specifies whether DdlUtils shall generate SQL to alter an existing
     * database rather than SQL for clearing it and creating it new.
     * 
     * @param alterTheDb
     *            <code>true</code> if SQL to alter the database shall be
     *            created
    ...
    /**
     * Determines whether SQL is generated to drop tables and the associated
    ...
    /**
     * Specifies whether SQL for dropping tables, external constraints, etc. is
     * created if necessary. Note that this is only relevant when
     * <code>alterDatabase</code> is <code>false</code>.
     * 
     * @param doDrops
     *            <code>true</code> if drops shall be performed if necessary
    ...
    public void execute(DatabaseTaskBase task, Database model)
        ...
        CreationParameters params = getFilteredParameters(model, platform
                .getName(), isCaseSensitive);
        ...
        try {
            ...
            if (shouldAlter) {
                if (getDataSource() == null) {
                    shouldAlter = false;
                    _log
                            .warn("Cannot alter the database because no database connection was specified."
                    ...
                    try {
                        ...
                        _log
                                .warn(
                                        "Could not establish a connection to the specified database, "
            ...
            if (shouldAlter) {
                Database currentModel = (getCatalogPattern() != null)
                        || (getSchemaPattern() != null) ? platform
                        .readModelFromDatabase("unnamed", getCatalogPattern(),
                                getSchemaPattern(), null) : platform
                        .readModelFromDatabase("unnamed");
