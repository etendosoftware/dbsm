 ...
 * necessary to create this schema in the database, to a file. Note that this SQL is database
 * specific and hence this subtask requires that for the enclosing task, either a data source is
 * specified (via the <code>database</code> sub element) or the <code>databaseType</code> attribute
...
public class WriteSchemaSqlToFileCommand extends DatabaseCommandWithCreationParameters {
  ...
  /**
   * Specifies whether DdlUtils shall generate SQL to alter an existing database rather than SQL for
   * clearing it and creating it new.
   * 
   * @param alterTheDb
   *          <code>true</code> if SQL to alter the database shall be created
        ...
        if (getDataSource() == null) {
          ...
          try {
            ...
            _log.warn("Could not establish a connection to the specified database, "
                + "so SQL for database creation will be generated instead.", ex);
          }
        }
      }
      if (shouldAlter) {
