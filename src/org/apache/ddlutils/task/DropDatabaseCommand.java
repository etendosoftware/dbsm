...
/**
 * Sub task for dropping the target database. Note that this is only supported
 * on some database platforms. See the database support documentation for
 * details on which platforms support this.<br/>
...
public class DropDatabaseCommand extends DatabaseCommand {
    ...
    public void execute(DatabaseTaskBase task, Database model)
        ...
        try {
            platform.dropDatabase(dataSource.getDriverClassName(), dataSource
            ...
            _log.error("Database platform " + platform.getName()
                    + " does not support database dropping via JDBC", ex);
        } catch (Exception ex) {
