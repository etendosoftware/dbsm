...
public abstract class DatabaseTaskBase extends Task {
    ...
    /**
     * Whether to use simple logging (that the Ant task configures itself via
     * the {@link #_verbosity} setting.
     */
     ...
     * <code>verbosity</code> setting) shall be used, or whether logging is
     * configured outside of the task (e.g. via a log4j properties file).
     * 
     * @param simpleLogging
     ...
     * is not able to derive the setting from the name of the used jdbc driver
     * or the jdbc connection url. If you have to specify this, please post your
     * jdbc driver and connection url combo to the user mailing list so that
     * DdlUtils can be enhanced to support this combo.<br/>
     ...
     * as table and column names). Most databases convert undelimited
     * identifiers to uppercase and ignore the case of identifiers when
     * performing any SQL command. Undelimited identifiers also cannot be
     * reserved words and can only contain alphanumerical characters and the
     * underscore.<br/>
     * These limitations do not exist for delimited identifiers where
     * identifiers have to be enclosed in double quotes. Delimited identifiers
     * can contain unicode characters, and even reserved words can be used as
     * identifiers. Please be aware though, that they always have to enclosed in
     * double quotes, and that the case of the identifier will be important in
     * every SQL command executed against the database.
     * 
    ...
    /**
     * Specifies whether DdlUtils shall sort the foreign keys of a table read
     * from a live database or leave them in the order in which they are
     * returned by the database/JDBC driver. Note that the sort is case
     * sensitive only if delimited identifier mode is on (
    ...
    /**
     * Specifies whether DdlUtils shall shut down the database after the task
     * has finished. This is mostly useful for embedded databases.
     * 
    ...
    public void execute() throws BuildException {
        ...
        ClassLoader sysClassLoader = Thread.currentThread()
                .getContextClassLoader();
        AntClassLoader newClassLoader = new AntClassLoader(getClass()
                .getClassLoader(), true);
