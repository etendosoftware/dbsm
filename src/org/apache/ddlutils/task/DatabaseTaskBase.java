...
public abstract class DatabaseTaskBase extends Task {
   ...
   * in double quotes. Delimited identifiers can contain unicode characters, and even reserved words
   * can be used as identifiers. Please be aware though, that they always have to enclosed in double
   * quotes, and that the case of the identifier will be important in every SQL command executed
  ...
  /**
   * Determines whether a table's foreign keys read from a live database shall be sorted
   * alphabetically. Is <code>false</code> by default.
   * 
   ...
   * leave them in the order in which they are returned by the database/JDBC driver. Note that the
   * sort is case sensitive only if delimited identifier mode is on (
  ...
  /**
   * Specifies whether DdlUtils shall shut down the database after the task has finished. This is
   * mostly useful for embedded databases.
   * 
