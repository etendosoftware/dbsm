 ...
 * create DTDs for data files, or write the SQL for creating a schema to a file. <br/>
 * Example:<br/>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This Ant build file snippet essentially creates a database, creates tables,
 * foreign keys etc. int it and then writes data into the newly created tables.
 * 
...
public class DdlToDatabaseTask extends DatabaseTaskBase {
    ...
    /**
     * Whether XML input files are validated against the internal or an external
     * DTD.
     */
    ...
    /**
     * Specifies whether DdlUtils shall use the embedded DTD for validating the
     * schema XML (if it matches
     * <code>http://db.apache.org/torque/dtd/database.dtd</code>). This is
     * especially useful in environments where no web access is possible or
     * desired.
     * 
     * @param useInternalDtd
     *            <code>true</code> if input files are to be validated against
     *            the internal DTD
    ...
    /**
     * Specifies whether XML input files should be validated against the DTD at
     * all.
     * 
     * @param validateXml
     *            <code>true</code> if input files are to be validated
     * @ant.not-required Default is <code>false</code> meaning that the XML is
    ...
    /**
     * Defines the single file that contains the database file. You can use this
     * instead of embedded <code>fileset</code> elements if you only have one
     * schema file.
     * 
     * @param schemaFile
     *            The schema
     * @ant.not-required Use either this or one or more embedded fileset
     *                   elements.
     */
        ...
        if (_singleSchemaFile != null) {
            ...
            for (Iterator it = _fileSets.iterator(); it.hasNext();) {
                ...
                for (int idx = 0; (files != null) && (idx < files.length); idx++) {
                    Database curModel = readSingleSchemaFile(reader, new File(
                    ...
                    if (model == null) {
                        ...
                        try {
                            ...
                            throw new BuildException(
                                    "Could not merge with schema from file "
                                            + files[idx] + ": "
    ...
    private Database readSingleSchemaFile(DatabaseIO reader, File schemaFile) {
        ...
        if (!schemaFile.isFile()) {
            _log.error("Path " + schemaFile.getAbsolutePath()
                    + " does not denote a file");
        } else if (!schemaFile.canRead()) {
            _log.error("Could not read schema file "
