...
/**
 * Task for performing operations on a live database. Sub tasks e.g. create the schema in the
 * database, drop database schemas, insert data into the database, create DTDs for data files, or
 * write the SQL for creating a schema to a file. <br/>
 * Example:<br/>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This Ant build file snippet essentially creates a database, creates tables, foreign keys etc. int
 * it and then writes data into the newly created tables.
 * 
...
public class DdlToDatabaseTask extends DatabaseTaskBase {
  ...
  /**
   * Specifies whether DdlUtils shall use the embedded DTD for validating the schema XML (if it
   * matches <code>http://db.apache.org/torque/dtd/database.dtd</code>). This is especially useful
   * in environments where no web access is possible or desired.
   * 
   * @param useInternalDtd
   *          <code>true</code> if input files are to be validated against the internal DTD
        ...
        for (int idx = 0; (files != null) && (idx < files.length); idx++) {
          ...
          if (model == null) {
            ...
            try {
              ...
              throw new BuildException("Could not merge with schema from file " + files[idx] + ": "
