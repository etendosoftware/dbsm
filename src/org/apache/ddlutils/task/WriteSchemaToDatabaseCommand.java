...
/**
 * Parses the schema XML files specified for the enclosing task, and creates the
 * corresponding schema in the database.
 * 
 * @version $Revision: 289996 $
 * @ant.task name="writeSchemaToDatabase"
 */
public class WriteSchemaToDatabaseCommand extends
    ...
    /**
     * Specifies whether DdlUtils shall alter an existing database rather than
     * clearing it and creating it new.
     * 
     * @param alterTheDb
     *            <code>true</code> if to alter the database
     ...
     * re-creating them (this implies <code>alterDatabase</code> is
     * <code>false</code>).
     * 
    ...
    /**
     * Specifies whether tables, external constraints, etc. can be dropped if
     * necessary. Note that this is only relevant when
     * <code>alterDatabase</code> is <code>false</code>.
     * 
     * @param doDrops
     *            <code>true</code> if drops shall be performed
     * @ant.not-required Per default database structures are dropped if
     *                   necessary
     */
    ...
    public void execute(DatabaseTaskBase task, Database model)
        ...
        // we're disabling the comment generation because we're writing directly
        // to the database
        platform.setSqlCommentsOn(false);
        try {
            if (isAlterDatabase()) {
                if ((getCatalogPattern() != null)
                        || (getSchemaPattern() != null)) {
                    platform.alterTables(getCatalogPattern(),
                            getSchemaPattern(), null, model, params, true);
                } else {
