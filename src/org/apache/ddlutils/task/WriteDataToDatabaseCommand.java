...
/**
 * Inserts the data defined by the data XML file(s) into the database. This requires the schema in
 * the database to match the schema defined by the XML files specified at the enclosing task.<br/>
 * DdlUtils will honor the order imposed by the foreign keys. Ie. first all required entries are
 * inserted, then the dependent ones. Obviously this requires that no circular references exist in
 * the schema (DdlUtils currently does not check this). Also, the referenced entries must be present
 * in the data, otherwise the task will fail. This behavior can be turned off via the
...
public class WriteDataToDatabaseCommand extends ConvertingDatabaseCommand {
  ...
  /**
   * Defines whether values for identity columns in the data XML shall be used instead of letting
   * the database define the value. Unless <code>ensureForeignKeyOrder</code> is set to false,
   * setting this to <code>false</code> (the default) does not affect foreign keys as DdlUtils will
   * automatically update the values of the columns of foreign keys pointing to the inserted row
   * with the database-created values.
   * 
   * @param useExplicitIdentityValues
   *          <code>true</code> if explicitly specified identity column values should be inserted
   *          instead of letting the database define the values for these columns
  ...
  /**
   * The maximum number of insert statements to combine in one batch. The number typically depends
   * on the JDBC driver and the amount of available memory.<br/>
  ...
  /**
   * Specifies whether batch mode shall be used for inserting the data. In batch mode, insert
   * statements for the same table are bundled together and executed as one statement. This can be a
   * lot faster than single insert statements but is not supported by all JDBC drivers/databases. To
   * achieve the highest performance, you should group the data in the XML file according to the
   * tables. This is because a batch insert only works for one table at a time. Thus when the table
   * changes in an entry in the XML file, the batch is committed and then a new one is started.
   * 
   * @param useBatchMode
