...
/**
 * A closure that determines a callback for the type of the object and calls it. Note that
 * inheritance is also taken into account. I.e. if the object is of type B which is a subtype of A,
 * and there is only a callback for type A, then this one will be invoked. If there is however also
 * a callback for type B, then only this callback for type B will be invoked and not the one for
 * type A.
 * 
 * @version $Revision: $
 */
public class CallbackClosure implements Closure {
   ...
   *          The parameter types. This array has to contain one <code>null</code> for the type of
   *          the object for which the callback is invoked. <code>null</code> or an empty array is
   *          regarded to be the same as an array containing a single <code>null</code>
   * @param parameters
   *          The actual arguments. The value at the placeholder position will be ignored. Can be
   *          <code>null</code> if no parameter types where given
   */
  public CallbackClosure(Object callee, String callbackName, Class[] parameterTypes,
      Object[] parameters) {
    _callee = callee;
