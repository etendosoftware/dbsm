...
public class diff_match_patch {
  ...
  /**
   * The size beyond which the double-ended diff activates. Double-ending is twice as fast, but less
   * accurate.
   */
  ...
  /**
   * When deleting a large block of text (over ~64 characters), how close does the contents have to
   * match the expected contents. (0.0 = perfection, 1.0 = very loose). Note that Match_Threshold
   ...
   * boundaries. Scores range from 5 (best) to 0 (worst).
   * 
   * @param one
   *          First string.
   * @param two
  ...
  public String diff_prettyHtml(LinkedList<Diff> diffs) {
    ...
    for (Diff aDiff : diffs) {
      ...
      case INSERT:
        html.append("<INS STYLE=\"background:#E6FFE6;\" TITLE=\"i=").append(i).append("\">")
            .append(text).append("</INS>");
        break;
      case DELETE:
        html.append("<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i=").append(i).append("\">")
