...
public abstract class Jdbc3Utils {
    ...
    public static int determineBooleanTypeCode()
        ...
        try {
            ...
            throw new UnsupportedOperationException(
                    "The jdbc type BOOLEAN is not supported");
        }
    }
    ...
    public static int determineDatalinkTypeCode()
        ...
        try {
            ...
            throw new UnsupportedOperationException(
                    "The jdbc type DATALINK is not supported");
        }
    }
     ...
     *         indicate an error
     */
    public static String getBatchResultMessage(String tableName, int rowIdx,
            int resultCode) {
        if (resultCode < 0) {
            try {
                if (resultCode == Statement.class.getField("SUCCESS_NO_INFO")
                        .getInt(null)) {
                    return null;
                } else if (resultCode == Statement.class.getField(
                        "EXECUTE_FAILED").getInt(null)) {
                    return "The batch insertion of row "
                            + rowIdx
                            + " into table "
                            + tableName
                            + " failed but the driver is able to continue processing";
                } else {
                    return "The batch insertion of row " + rowIdx
                            + " into table " + tableName
                            + " returned an undefined status value "
                            + resultCode;
                }
            } catch (Exception ex) {
                throw new UnsupportedOperationException(
                        "The batch result codes are not supported");
            }
        } else {
            return null;
        }
