...
/**
 * Little helper class providing functions for dealing with the newer JDBC functionality in a way
 * that is safe to compile with Java 1.3.
 * 
...
public abstract class Jdbc3Utils {
   ...
   * 
   * @return <code>true</code> if SUCCESS_NO_INFO and EXECUTE_FAILED are available in the
  ...
  /**
   * Returns the logging message corresponding to the given result code of a batch message. Note
   * that these code values are only available in JDBC 3 and newer (see {@link java.sql.Statement}
   * for details).
   * 
   * @param tableName
   *          The name of the table that the batch update/insert was performed on
   * @param rowIdx
