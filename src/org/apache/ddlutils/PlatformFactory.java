 ...
 * insensitive database name. Note that this is a convenience class as the
 * platforms can also simply be created via their constructors.
 * 
...
public class PlatformFactory {
     ...
     *         supported
     */
    public static synchronized Platform createNewPlatformInstance(
            String databaseName) throws DdlUtilsException {
        Class platformClass = (Class) getPlatforms().get(
                databaseName.toLowerCase());

        try {
            return platformClass != null ? (Platform) platformClass
                    .newInstance() : null;
        } catch (Exception ex) {
            throw new DdlUtilsException(
                    "Could not create platform for database " + databaseName,
                    ex);
        }
    }
     ...
     * method that uses
     * {@link PlatformUtils#determineDatabaseType(String, String)} to determine
     * the parameter for {@link #createNewPlatformInstance(String)}. Note that
     * no database connection is established when using this method.
     * 
     * @param jdbcDriver
     *            The jdbc driver
     ...
     */
    public static synchronized Platform createNewPlatformInstance(
            String jdbcDriver, String jdbcConnectionUrl)
            throws DdlUtilsException {
        return createNewPlatformInstance(new PlatformUtils()
                .determineDatabaseType(jdbcDriver, jdbcConnectionUrl));
    }

    /**
     * Creates a new platform for the specified database. This is a shortcut
     * method that uses {@link PlatformUtils#determineDatabaseType(DataSource)}
     * to determine the parameter for {@link #createNewPlatformInstance(String)}
     * . Note that this method sets the data source at the returned platform
     ...
     */
    public static synchronized Platform createNewPlatformInstance(
            DataSource dataSource) throws DdlUtilsException {
    ...
    public static synchronized Platform createNewPlatformInstance(
            ...
            throws DdlUtilsException {
        Platform platform = createNewPlatformInstance(new PlatformUtils()
                .determineDatabaseType(dataSource, username, password));
     ...
     *            The platform name
     * @param platformClass
     *            The platform class which must implement the {@link Platform}
     *            interface
     */
    ...
    private static void registerPlatforms() {
        ...
        addPlatform(_platforms, SybaseASE15Platform.DATABASENAME,
                SybaseASE15Platform.class);
    }
     ...
     *            interface
     */
    private static synchronized void addPlatform(Map platformMap,
            String platformName, Class platformClass) {
