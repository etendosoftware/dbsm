...
public abstract class SqlBuilder {
    ...
    public void setValueLocale(String localeStr) {
        if (localeStr != null) {
            ...
            if (language != null) {
                ...
                setValueDateFormat(DateFormat.getDateInstance(DateFormat.SHORT,
                        locale));
                setValueTimeFormat(DateFormat.getTimeInstance(DateFormat.SHORT,
                        locale));
    ...
    public void createTables(Database database, CreationParameters params,
        ...
        for (int idx = 0; idx < database.getTableCount(); idx++) {
            ...
            createTable(database, table, params == null ? null : params
                    .getParametersFor(table));
    ...
    public void alterDatabase(Database currentModel, Database desiredModel,
        ...
        for (int i = 0; i < desiredModel.getFunctionCount(); i++)
            desiredModel.getFunction(i).setTranslation(
                    _PLSQLFunctionTranslation);
    ...
    public void alterData(Database model, Vector<Change> changes)
            throws IOException {
            ...
            for (int i = 0; i < model.getTableCount(); i++) {
                ...
                for (int j = 0; j < fksTable.length; j++) {
                    ...
                    if (fk.getOnDelete() != null
                            && fk.getOnDelete().contains("cascade")) {
    ...
    public void alterDatabasePostScript(Database currentModel,
        ...
        while (it.hasNext()) {
            ...
            if (change instanceof AddColumnChange) {
                ...
                processChange(currentModel, desiredModel, params,
                        ((AddUniqueChange) change));
            } else if (change instanceof AddCheckChange) {
                processChange(currentModel, desiredModel, params,
                        ((AddCheckChange) change));
            }
        ...
        Predicate predicate = new MultiInstanceofPredicate(new Class[] {
                RemovePrimaryKeyChange.class, AddPrimaryKeyChange.class,
                PrimaryKeyChange.class, RemoveColumnChange.class,
                AddColumnChange.class, ColumnOrderChange.class,
                ColumnAutoIncrementChange.class,
                ColumnDefaultValueChange.class,
                ColumnOnCreateDefaultValueChange.class,
                ColumnRequiredChange.class, ColumnDataTypeChange.class,
        ...
        for (int i = 0; i < desiredModel.getTableCount(); i++) {
            ...
            while (itChanges.hasNext()) {
                ...
                if (currentChange.getChangedTable().getName().equalsIgnoreCase(
                    ...
                    if (currentChange instanceof AddColumnChange) {
                        newColumnsThisTable
                                .add((AddColumnChange) currentChange);
                        newColumn = true;
                    }
            ...
            recreated = willBeRecreated(desiredModel.getTable(i),
                    changesOfTable);
            ...
            for (int j = 0; j < newColumns.size(); j++) {
                ...
                if (table.getName().equalsIgnoreCase(
                        desiredModel.getTable(i).getName())) {
                    Table tempTable = getTemporaryTableFor(desiredModel, change
                            .getChangedTable());
                    if (change.getNewColumn().getOnCreateDefault() != null) {
                        executeOnCreateDefault(table, tempTable, change
                                .getNewColumn(), recreated);
                        writeColumnCommentStmt(currentModel, change
                                .getChangedTable(), change.getNewColumn());
                    }
                }
            }
            if (recreated) {
                writeExternalPrimaryKeysCreateStmt(desiredModel.getTable(i),
                        desiredModel.getTable(i).getPrimaryKey(), desiredModel
                                .getTable(i).getPrimaryKeyColumns());
        ...
        addRelevantFKsFromUnchangedTables(currentModel, desiredModel,
                changesPerTable.keySet(), unchangedFKs);

        for (Iterator tableFKIt = unchangedFKs.entrySet().iterator(); tableFKIt
                .hasNext();) {
            Map.Entry entry = (Map.Entry) tableFKIt.next();
            Table targetTable = desiredModel.findTable((String) entry.getKey(),
                    caseSensitive);

            for (Iterator fkIt = ((List) entry.getValue()).iterator(); fkIt
                    .hasNext();) {
    ...
    public boolean willBeRecreated(Table table, Vector<TableChange> changes) {
        ...
        if (changes.size() > 1)
            ...
            return false;
        boolean recreated = false;
        if (changes.get(0) instanceof AddPrimaryKeyChange
                || changes.get(0) instanceof ColumnOnCreateDefaultValueChange)
            return false;
        else
            return true;
    }

    public void executeStandardDefault(Table table, Column col)
            throws IOException {
        if (col.getDefaultValue() != null && !col.getDefaultValue().equals("")) {
            println("UPDATE " + table.getName() + " SET " + col.getName() + "="
                    + getDefaultValue(col) + " WHERE " + col.getName()
                    + " IS NULL");
            printEndOfStatement();
        }
    }
    ...
    public void executeOnCreateDefault(Table table, Table tempTable,
        ...
        for (int i = 0; i < pks1.length; i++) {
            if (i > 0)
                pk += " AND ";
            pk += "TO_CHAR(" + table.getName() + "." + pks1[i].getName()
                    + ")=TO_CHAR(" + tempTable.getName() + "."
                    + pks1[i].getName() + ")";
        }
     ...
     * {@link org.apache.ddlutils.alteration.RemoveIndexChange} come first to
     * allow for e.g. subsequent primary key changes or column removal.</li>
     * <li>{@link org.apache.ddlutils.alteration.RemoveTableChange} comes after
     ...
     * {@link org.apache.ddlutils.alteration.AddIndexChange} come last after
    ...
    protected void processChanges(Database currentModel, Database desiredModel,
            List changes, CreationParameters params, boolean createConstraints)
            throws IOException {
        CallbackClosure callbackClosure = new CallbackClosure(this,
                "processChange", new Class[] { Database.class, Database.class,
                        CreationParameters.class, null }, new Object[] {
                        currentModel, desiredModel, params, null });

        // 1st pass: removing external constraints and indices
        applyForSelectedChanges(changes, new Class[] {
                RemoveForeignKeyChange.class, RemoveUniqueChange.class,
                RemoveIndexChange.class, RemoveCheckChange.class },
                callbackClosure);

        // 2nd pass: removing tables and views and functions and triggers
        applyForSelectedChanges(changes,
                new Class[] { RemoveViewChange.class }, callbackClosure);
        applyForSelectedChanges(changes, new Class[] {
                RemoveTriggerChange.class, RemoveFunctionChange.class,
                RemoveTableChange.class, RemoveSequenceChange.class },
                callbackClosure);
        ...
        processTableStructureChanges(currentModel, desiredModel, params,
                CollectionUtils.select(changes, predicate), CollectionUtils
                        .select(changes, predicatetriggers));

        // 4th pass: adding tables
        applyForSelectedChanges(changes, new Class[] { AddTableChange.class },
                callbackClosure);

        // 5th pass: adding external constraints and indices
        if (createConstraints) {
            applyForSelectedChanges(changes, new Class[] {
                    AddForeignKeyChange.class, AddUniqueChange.class,
                    AddIndexChange.class, AddCheckChange.class },
                    callbackClosure);
        }

        applyForSelectedChanges(changes,
                new Class[] { AddSequenceChange.class }, callbackClosure);

        applyForSelectedChanges(changes,
                new Class[] { AddFunctionChange.class }, callbackClosure);

        applyForSelectedChanges(changes, new Class[] { AddViewChange.class },
                callbackClosure);

        applyForSelectedChanges(changes,
                new Class[] { AddTriggerChange.class }, callbackClosure);
    }
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveForeignKeyChange change)
            throws IOException {
        writeExternalForeignKeyDropStmt(change.getChangedTable(), change
                .getForeignKey());
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveUniqueChange change)
            throws IOException {
        writeExternalUniqueDropStmt(change.getChangedTable(), change
                .getUnique());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveIndexChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveCheckChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveTableChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddTableChange change)
            throws IOException {
        createTable(desiredModel, change.getNewTable(), params == null ? null
                : params.getParametersFor(change.getNewTable()));
        writeExternalPrimaryKeysCreateStmt(change.getNewTable(), change
                .getNewTable().getPrimaryKey(), change.getNewTable()
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveSequenceChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddSequenceChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveViewChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveFunctionChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddFunctionChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, RemoveTriggerChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddTriggerChange change)
            throws IOException {
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddForeignKeyChange change)
            throws IOException {
        writeExternalForeignKeyCreateStmt(desiredModel, change
                .getChangedTable(), change.getNewForeignKey());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddUniqueChange change)
            throws IOException {
        writeExternalUniqueCreateStmt(change.getChangedTable(), change
                .getNewUnique());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddIndexChange change)
            throws IOException {
        writeExternalIndexCreateStmt(change.getChangedTable(), change
                .getNewIndex());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
     ...
     */
    protected void processChange(Database currentModel, Database desiredModel,
            CreationParameters params, AddCheckChange change)
            throws IOException {
        writeExternalCheckCreateStmt(change.getChangedTable(), change
                .getNewCheck());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
    ...
    protected void processTableStructureChanges(Database currentModel,
            Database desiredModel, CreationParameters params,
            Collection changes, Collection triggerchanges) throws IOException {
        ...
        for (Iterator tableFKIt = unchangedFKs.entrySet().iterator(); tableFKIt
            ...
            for (Iterator fkIt = ((List) entry.getValue()).iterator(); fkIt
                    .hasNext();) {
                writeExternalForeignKeyDropStmt(targetTable, (ForeignKey) fkIt
                        .next());
            }
        }
        ...
        for (Iterator tableChangeIt = changesPerTable.entrySet().iterator(); tableChangeIt
                .hasNext();) {
            Map.Entry entry = (Map.Entry) tableChangeIt.next();
            Table targetTable = desiredModel.findTable((String) entry.getKey(),
                    caseSensitive);

            processTableStructureChanges(copyOfCurrentModel, desiredModel,
                    (String) entry.getKey(), params == null ? null : params
                            .getParametersFor(targetTable), (List) entry
                            .getValue(), unchangedTriggers);
        }
        ...
        for (Iterator tableFKIt = unchangedFKs.entrySet().iterator(); tableFKIt
            ...
            for (Iterator fkIt = ((List) entry.getValue()).iterator(); fkIt
                    .hasNext();) {
                currentModel.findTable((String) entry.getKey(), caseSensitive)
                        .removeForeignKey((ForeignKey) fkIt.next());
                // writeExternalForeignKeyCreateStmt(desiredModel, targetTable,
                // (ForeignKey)fkIt.next());
            }
        }
    ...
    private List getUnchangedForeignKeys(Database currentModel,
            Database desiredModel, String tableName) {
    ...
    private void addRelevantFKsFromUnchangedTables(Database currentModel,
        ...
        for (int tableIdx = 0; tableIdx < desiredModel.getTableCount(); tableIdx++) {
            ...
            if ((sourceTable != null)
                    && !namesOfKnownChangedTables.contains(name)) {
                for (int fkIdx = 0; fkIdx < targetTable.getForeignKeyCount(); fkIdx++) {
                    ...
                    if ((sourceFk != null)
                            && namesOfKnownChangedTables.contains(refName)) {
                        if (relevantFks == null) {
     ...
     */
    protected void processTableStructureChanges(Database currentModel,
            Database desiredModel, String tableName, Map parameters,
            List changes, Set<String> unchangedtriggers) throws IOException {
        ...
        if (!changes.isEmpty()) {
            ...
            for (int i = 0; i < desiredModel.getTriggerCount(); i++) {
                ...
                if (unchangedtriggers.contains(t.getName())
                        && t.getTable().equals(tableName)) {
                    triggers.add(t);
                }
            }
             ...
             * it.hasNext();) { TableChange change = (TableChange)it.next();
             * 
             ...
             * addColumnChange.getNewColumn().getName()); canMigrateData =
             * false; } } }
             */
            ...
            {
                Table tempTable = getTemporaryTableFor(currentModel,
                        sourceTable);
     ...
     *            The desired table
     * @param parameters
     *            The creation parameters for the table
     * @param changes
    ...
    protected void processTableStructureChanges(Database currentModel,
        ...
        if (changes.size() == 1) {
            ...
            if (change instanceof AddPrimaryKeyChange) {
                ...
                processChange(currentModel, desiredModel,
                        (ColumnOnCreateDefaultValueChange) change);
                changes.clear();
            }
        }
    }
        ...
        for (int i = 0; i < table.getColumnCount(); i++) {
            ...
            if (column.isRequired()) {
                println("ALTER TABLE " + table.getName() + " MODIFY "
                        + getColumnName(column) + " " + getSqlType(column)
                        + " NULL");
                printEndOfStatement();
            }
    ...
    public void processDataChanges(Database database,
            DatabaseData databaseData, List changes) throws IOException {
        for (int i = 0; i < changes.size(); i++) {
            if (changes.get(i) instanceof AddRowChange) {
                AddRowChange change = (AddRowChange) changes.get(i);
                DynaBean row = change.getRow();
                DynaProperty[] properties = database.getDynaClassFor(row)
                        .getDynaProperties();
                HashMap parameters = new HashMap();

                for (int idx = 0; idx < properties.length; idx++) {
                    parameters.put(properties[idx].getName(), row
                ...
                parameters.put(change.getColumn().getName(), change
                        .getNewValue());
    ...
    public void generateDataInsertions(Database database,
            DatabaseData databaseData) throws IOException {
        for (int i = 0; i < database.getTableCount(); i++) {
            Table table = database.getTable(i);
            Vector<DynaBean> rowsTable = databaseData.getRowsFromTable(table
                    .getName());
            if (rowsTable != null) {
     ...
     * @return The table
     */
    protected Table getRealTargetTableFor(Database targetModel,
            Table sourceTable, Table targetTable) throws IOException {
        Table table = new Table();
        ...
        for (int idx = 0; idx < targetTable.getUniqueCount(); idx++) {
            ...
            Unique sourceUnique = sourceTable.findUnique(
                    targetUnique.getName(), caseSensitive);

            if (sourceUnique != null) {
                if ((caseSensitive && sourceUnique.equals(targetUnique))
                        || (!caseSensitive && sourceUnique
                                .equalsIgnoreCase(targetUnique))) {
        ...
        for (int idx = 0; idx < targetTable.getIndexCount(); idx++) {
            ...
            if (sourceIndex != null) {
                if ((caseSensitive && sourceIndex.equals(targetIndex))
                        || (!caseSensitive && sourceIndex
                                .equalsIgnoreCase(targetIndex))) {
        ...
        for (int idx = 0; idx < targetTable.getCheckCount(); idx++) {
            ...
            Check sourceCheck = sourceTable.findCheck(targetCheck.getName(),
                    caseSensitive);

            if (sourceCheck != null) {
                if ((caseSensitive && sourceCheck.equals(targetCheck))
                        || (!caseSensitive && sourceCheck
                                .equalsIgnoreCase(targetCheck))) {
    ...
    protected void writeCopyDataStatement(Table sourceTable, Table targetTable)
        ...
        print(" (");
        for (Iterator columnIt = columns.keySet().iterator(); columnIt
                .hasNext();) {
        ...
        print(") SELECT ");
        for (Iterator columnsIt = columns.entrySet().iterator(); columnsIt
                .hasNext();) {
            Map.Entry entry = (Map.Entry) columnsIt.next();

            writeCastExpression((Column) entry.getKey(), (Column) entry
                    .getValue());
    ...
    protected void processChange(Database currentModel, Database desiredModel,
        ...
        writeExternalPrimaryKeysCreateStmt(change.getChangedTable(), change
                .getprimaryKeyName(), change.getPrimaryKeyColumns());
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }

    protected void processChange(Database currentModel, Database desiredModel,
            ColumnOnCreateDefaultValueChange change) throws IOException {
        writeColumnCommentStmt(currentModel, change.getChangedTable(), change
                .getChangedColumn());
        printEndOfStatement();
        change.apply(currentModel, getPlatform().isDelimitedIdentifierModeOn());
    }
    ...
    protected ForeignKey findCorrespondingForeignKey(Table table, ForeignKey fk) {
        ...
        for (int fkIdx = 0; fkIdx < table.getForeignKeyCount(); fkIdx++) {
            ...
            if ((!checkCurFkName || areEqual(fk.getName(), curFk.getName(),
                    caseMatters))
                    && areEqual(fk.getForeignTableName(), curFk
                            .getForeignTableName(), caseMatters)) {
                curRefs.clear();
                ...
                if (curRefs.size() == refs.length) {
                    for (int refIdx = 0; refIdx < refs.length; refIdx++) {
                        ...
                        for (int curRefIdx = 0; !found
                                && (curRefIdx < curRefs.size()); curRefIdx++) {
                            Reference curRef = (Reference) curRefs
                                    .get(curRefIdx);

                            if ((caseMatters && refs[refIdx].equals(curRef))
                                    || (!caseMatters && refs[refIdx]
                                            .equalsIgnoreCase(curRef))) {
     ...
     */
    protected boolean areEqual(String string1, String string2,
            boolean caseMatters) {
    ...
    public void createExternalForeignKeys(Database database, Table table)
        ...
        if (!getPlatformInfo().isForeignKeysEmbedded()) {
            for (int idx = 0; idx < table.getForeignKeyCount(); idx++) {
                writeExternalForeignKeyCreateStmt(database, table, table
                        .getForeignKey(idx));
            }
        }
    }
        ...
        if (genPlaceholders) {
            ...
            for (int idx = 0; idx < table.getColumnCount(); idx++) {
                ...
                if (columnValues.containsKey(column.getName())) {
                    ...
                    buffer.append(getValueAsString(column, columnValues
                            .get(column.getName())));
                    addComma = true;
                }
            }
    ...
    public String getUpdateSql(Table table, Map columnValues,
        ...
        for (int idx = 0; idx < table.getColumnCount(); idx++) {
            ...
            if (!column.isPrimaryKey()
                    && columnValues.containsKey(column.getName())) {
                if (addSep) {
                    buffer.append(", ");
                }
                buffer.append(getDelimitedIdentifier(column.getName()));
     ...
     */
    public String getDeleteSql(Table table, Map pkValues,
            boolean genPlaceholders) {
        ...
        if ((pkValues != null) && !pkValues.isEmpty()) {
            ...
            for (Iterator it = pkValues.entrySet().iterator(); it.hasNext();) {
                ...
                buffer
                        .append(getDelimitedIdentifier(entry.getKey()
                                .toString()));
                buffer.append(" = ");
                if (genPlaceholders) {
                    buffer.append("?");
                } else {
                    buffer.append(column == null ? entry.getValue()
                            : getValueAsString(column, entry.getValue()));
                }
                addSep = true;
            }
        }
    ...
    public String shortenName(String name, int desiredLength) {
        ...
        _log.warn("Warning: Name of object " + name + " is too long ("
                + originalLength + " characters; maximum length: "
        ...
        if (((startCut == 0) || (name.charAt(startCut - 1) != '_'))
                && ((startCut + delta + 1 == originalLength) || (name
                        .charAt(startCut + delta + 1) != '_'))) {
     ...
     *            The table
     */
    public void writeDeleteTable(Database database, Table table)
            throws IOException {
        print("DELETE FROM ");
        printIdentifier(getStructureObjectName(table));
        printEndOfStatement();
    ...
    public void writeDeleteTable(Database database, String table,
            String sqlfilter) throws IOException {
        print("DELETE FROM ");
    ...
    protected String getSqlType(Column column) {
        ...
        if (sizeSpec != null) {
            if (getPlatformInfo().hasSize(column.getTypeCode())) {
                ...
                sqlType.append(")");
            } else if (getPlatformInfo().hasPrecisionAndScale(
                    column.getTypeCode())) {
                sqlType.append("(");
    ...
    protected String getNativeType(Column column) {
        String nativeType = getPlatformInfo().getNativeType(
                column.getTypeCode());
    ...
    protected String escapeStringValue(String value) {
        ...
        for (Iterator it = _charSequencesToEscape.entrySet().iterator(); it
                .hasNext();) {
            Map.Entry entry = (Map.Entry) it.next();

            result = StringUtils.replace(result, (String) entry.getKey(),
    ...
    protected boolean isValidDefaultValue(String defaultSpec, int typeCode) {
        return (defaultSpec != null)
                && ((defaultSpec.length() > 0) || (!TypeMap
                        .isNumericType(typeCode) && !TypeMap
     ...
     */
    protected String getDefaultValue(Object defaultValue, int typeCode)
            throws IOException {
    ...
    protected boolean columnsDiffer(Column currentColumn, Column desiredColumn) {
        ...
        boolean sizeMatters = getPlatformInfo().hasSize(
    ...
    public String getForeignKeyName(Table table, ForeignKey fk) {
        ...
        if (needsName) {
            _log
                    .warn("Encountered a foreign key in table "
                            + table.getName()
                            + " that has no name. "
     ...
     */
    public String getConstraintName(String prefix, Table table,
            String secondPart, String suffix) {
        StringBuffer result = new StringBuffer();
    ...
    protected void writeExternalPrimaryKeysCreateStmt(Table table,
            String primaryKeyName, Column[] primaryKeyColumns)
            throws IOException {
        if ((primaryKeyColumns.length > 0)
                && shouldGeneratePrimaryKeys(primaryKeyColumns)) {
            print("ALTER TABLE ");
     ...
     *            The table
     */
    protected void writeExternalUniquesCreateStmt(Table table)
            throws IOException {
     ...
     *            The unique
     */
    protected void writeExternalUniqueCreateStmt(Table table, Unique unique)
            throws IOException {
        if (getPlatformInfo().isIndicesSupported()) {
            if (unique.getName() == null) {
                ...
                for (int idx = 0; idx < unique.getColumnCount(); idx++) {
                    ...
                    if (col == null) {
                        ...
                        // exception
                        throw new ModelException("Invalid column '"
                                + idxColumn.getName() + "' on unique "
                                + unique.getName() + " for table "
                                + table.getName());
                    }
                    if (idx > 0) {
                        print(", ");
     ...
     *            The unique
     */
    protected void writeEmbeddedUniqueCreateStmt(Table table, Unique unique)
            throws IOException {
     ...
     *            The unique to drop
     */
    public void writeExternalUniqueDropStmt(Table table, Unique unique)
            throws IOException {
        print("ALTER TABLE ");
        printIdentifier(getStructureObjectName(table));
        ...
        for (int idx = 0; idx < table.getIndexCount(); idx++) {
            ...
            if (!index.isUnique() && !getPlatformInfo().isIndicesSupported()) {
                throw new ModelException(
                        "Platform does not support non-unique indices");
            }
            ...
            if (index.getName() == null) {
                ...
                for (int idx = 0; idx < index.getColumnCount(); idx++) {
                    ...
                    if (col == null) {
                        ...
                        // exception
                        throw new ModelException("Invalid column '"
                                + idxColumn.getName() + "' on index "
                                + index.getName() + " for table "
                                + table.getName());
                    }
                    if (idx > 0) {
                        print(", ");
    ...
    protected void writeEmbeddedIndexCreateStmt(Table table, Index index)
        ...
        for (int idx = 0; idx < index.getColumnCount(); idx++) {
            ...
            if (col == null) {
                ...
                // exception
                throw new ModelException("Invalid column '"
                        + idxColumn.getName() + "' on index " + index.getName()
    ...
    protected void writeExternalForeignKeyCreateStmt(Database database,
        ...
        if (key.getForeignTableName() == null) {
            ...
            writeLocalReferences(key);
            print(") REFERENCES ");
            printIdentifier(shortenName(key.getForeignTableName(),
     ...
     *            The foreign key
     */
    protected void writeForeignKeyOnUpdateOption(ForeignKey key)
            throws IOException {
     ...
     *            The foreign key
     */
    protected void writeForeignKeyOnDeleteOption(ForeignKey key)
            throws IOException {
     ...
     *            The table
     */
    protected void writeExternalChecksCreateStmt(Table table)
            throws IOException {
     ...
     *            The check
     */
    protected void writeExternalCheckCreateStmt(Table table, Check check)
            throws IOException {
    ...
    protected void writeEmbeddedCheckCreateStmt(Table table, Check check)
            throws IOException {
     ...
     *            The check to drop
     */
    public void writeExternalCheckDropStmt(Table table, Check check)
            throws IOException {
    ...
    protected void createSequence(Sequence sequence) throws IOException {
        ...
        if (getPlatformInfo().isSequencesSupported()) {
            if (sequence.getName() == null) {
                ...
                printStartOfStatement("SEQUENCE",
                        getStructureObjectName(sequence));
    ...
    protected void createFunction(Function function) throws IOException {
        ...
        if (getPlatformInfo().isFunctionsSupported()) {
            ...
            if (function.getName() == null) {
                ...
                printStartOfStatement("FUNCTION",
                        getStructureObjectName(function));
    ...
    /**
     * Gets the end clause for a function with no parameters. Usually is empty
     * and for Postgre for example is "()"
     */
    ...
    protected String getDefaultValue(ValueObject identifier) throws IOException {
        ...
        if (identifier.isDefaultFunction()) {
            return getNativeFunction(identifier.getDefaultValue(), identifier
                    .getTypeCode());
        } else {
            ...
            if (parsedDefault == null) {
                ...
                if (!getPlatformInfo().isDefaultValuesForLongTypesSupported()
                        && ((identifier.getTypeCode() == Types.LONGVARBINARY) || (identifier
                ...
                if (isValidDefaultValue(identifier.getDefaultValue(),
                        identifier.getTypeCode())) {
                    return getDefaultValue(getNativeDefaultValue(identifier),
                            identifier.getTypeCode());
                } else {
                    return null;
                }
     ...
     *            The trigger
     */
    public void createTrigger(Database database, Trigger trigger)
            throws IOException {
        ...
        if (getPlatformInfo().isTriggersSupported()) {
            ...
            if (trigger.getName() == null) {
                ...
                print("ON ");
                printIdentifier(shortenName(trigger.getTable(),
                        getMaxTableNameLength()));// database.findTable(trigger.getTable())));
     ...
     *            The trigger
     */
    public void dropTrigger(Database database, Trigger trigger)
            throws IOException {
    ...
    /**
     * Returns the structure name. This method takes care of length limitations
     * imposed by some databases.
     * 
     * @param obj
     *            The structure
    ...
    protected void printStartOfStatement(String type, String statementName)
            throws IOException {
        printComment("-----------------------------------------------------------------------");
    ...
    public void getConfigScript(Database database, Vector<Change> changes)
            throws IOException {
        for (Change change : changes) {
    ...
    public void printColumnDataChange(Database database, ColumnDataChange change)
            throws IOException {
    ...
    public void printAddRowChangeChange(Database model, AddRowChange change)
            throws IOException {
    ...
    public void printRemoveRowDALChange(Database model,
            RemoveRowDALChange change) throws IOException {
        Table table = change.getTable();
