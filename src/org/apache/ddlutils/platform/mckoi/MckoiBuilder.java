...
public class MckoiBuilder extends SqlBuilder {
    ...
    protected void writeColumnDefaultValueStmt(Table table, Column column)
        ...
        if (column.isDefaultFunction()) {
            print(" DEFAULT ");
            print(getNativeFunction(column.getDefaultValue(), column
                    .getTypeCode()));
        } else {
    ...
    protected void writeColumnDefaultValue(Table table, Column column)
        ...
        if (column.isAutoIncrement()) {
            ...
            print(getDefaultValue(getNativeDefaultValue(column), column
                    .getTypeCode()));
        }
    }
    ...
    public String getSelectLastIdentityValues(Table table) {
        ...
        if (columns.length > 0) {
            ...
            for (int idx = 0; idx < columns.length; idx++) {
                ...
                result.append(getConstraintName("seq", table, columns[idx]
                        .getName(), null));
                result.append("')");
            }
    ...
    protected void processTableStructureChanges(Database currentModel,
        ...
        for (Iterator it = changes.iterator(); it.hasNext();) {
            ...
            if (change instanceof ColumnAutoIncrementChange) {
                Column column = ((ColumnAutoIncrementChange) change)
                        .getColumn();
                ...
                if (!column.isAutoIncrement()) {
                    ...
                    createAutoIncrementSequence(autoIncrChange
                            .getChangedTable(), autoIncrChange.getColumn());
                }
            } else if (change instanceof AddColumnChange) {
                AddColumnChange addColumnChange = (AddColumnChange) change;

                if (addColumnChange.getNewColumn().isAutoIncrement()) {
                    createAutoIncrementSequence(addColumnChange
                            .getChangedTable(), addColumnChange.getNewColumn());
                }
            }
        }
        ...
        for (Iterator it = changes.iterator(); it.hasNext();) {
            ...
            if (change instanceof ColumnAutoIncrementChange) {
                ...
                if (removeColumnChange.getColumn().isAutoIncrement()) {
                    dropAutoIncrementSequence(removeColumnChange
                            .getChangedTable(), removeColumnChange.getColumn());
                }
            }
        }
        changes.clear();
