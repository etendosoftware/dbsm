...
public class OracleModelLoader extends ModelLoaderBase {
	...
	protected void initMetadataSentences() throws SQLException {
        ...
        _stmt_pkname_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'P' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%'");
        ...
        _stmt_listcolumns_prefix = _connection.prepareStatement("SELECT COLUMN_NAME, DATA_TYPE, CHAR_COL_DECL_LENGTH, DATA_LENGTH ,DATA_PRECISION, DATA_SCALE, NULLABLE, DATA_DEFAULT FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND upper(COLUMN_NAME) LIKE 'EM_"+_prefix+"_%' ORDER BY COLUMN_ID");
        ...
        _stmt_listchecks_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME, SEARCH_CONDITION FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'C' AND GENERATED = 'USER NAME' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' ORDER BY CONSTRAINT_NAME");
        ...
        _stmt_listuniques_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'U' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' ORDER BY CONSTRAINT_NAME");
        ...
        if(_prefix!=null)
        	sql+=" AND UPPER(VIEW_NAME) LIKE '"+_prefix+"_%'";
        ...
        {
        	if(!sql.contains("WHERE")) sql+=" WHERE 1=1";
        	sql+=" AND UPPER(TRIGGER_NAME) LIKE '"+_prefix+"_%'";
        }
        ...
        if(_prefix!=null)
        	sql+=" AND UPPER(NAME) LIKE '"+_prefix+"_%'";
