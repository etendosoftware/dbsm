...
public class OracleModelLoader extends ModelLoaderBase {
	...
	protected void initMetadataSentences() throws SQLException {
        ...
        _stmt_pkname_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'P' AND TABLE_NAME = ? AND (upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(CONSTRAINT_NAME)||UPPER(TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"')))");
        _stmt_pkname_noprefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'P' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) NOT LIKE 'EM_%'");
        ...
        _stmt_listcolumns_prefix = _connection.prepareStatement("SELECT COLUMN_NAME, DATA_TYPE, CHAR_COL_DECL_LENGTH, DATA_LENGTH ,DATA_PRECISION, DATA_SCALE, NULLABLE, DATA_DEFAULT FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND (upper(COLUMN_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(COLUMN_NAME)||UPPER(TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"'))) ORDER BY COLUMN_ID");
        _stmt_listcolumns_noprefix = _connection.prepareStatement("SELECT COLUMN_NAME, DATA_TYPE, CHAR_COL_DECL_LENGTH, DATA_LENGTH ,DATA_PRECISION, DATA_SCALE, NULLABLE, DATA_DEFAULT FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND upper(COLUMN_NAME) NOT LIKE 'EM_%' ORDER BY COLUMN_ID");
        ...
        _stmt_listchecks_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME, SEARCH_CONDITION FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'C' AND GENERATED = 'USER NAME' AND TABLE_NAME = ? AND (upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(CONSTRAINT_NAME)||UPPER(TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"'))) ORDER BY CONSTRAINT_NAME");
        _stmt_listchecks_noprefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME, SEARCH_CONDITION FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'C' AND GENERATED = 'USER NAME' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) NOT LIKE 'EM_%' ORDER BY CONSTRAINT_NAME");
        ...
        _stmt_listfks_prefix = _connection.prepareStatement("SELECT C.CONSTRAINT_NAME, C2.TABLE_NAME, C.DELETE_RULE, 'NO ACTION' FROM USER_CONSTRAINTS C, USER_CONSTRAINTS C2 WHERE C.R_CONSTRAINT_NAME = C2.CONSTRAINT_NAME AND C.CONSTRAINT_TYPE = 'R' AND C.TABLE_NAME = ? AND (upper(C.CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(C.CONSTRAINT_NAME)||UPPER(C2.TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"'))) ORDER BY C.CONSTRAINT_NAME");
        _stmt_listfks_noprefix = _connection.prepareStatement("SELECT C.CONSTRAINT_NAME, C2.TABLE_NAME, C.DELETE_RULE, 'NO ACTION' FROM USER_CONSTRAINTS C, USER_CONSTRAINTS C2 WHERE C.R_CONSTRAINT_NAME = C2.CONSTRAINT_NAME AND C.CONSTRAINT_TYPE = 'R' AND C.TABLE_NAME = ? AND upper(C.CONSTRAINT_NAME) NOT LIKE 'EM_%' ORDER BY C.CONSTRAINT_NAME");
        ...
        _stmt_listindexes_prefix = _connection.prepareStatement("SELECT INDEX_NAME, UNIQUENESS FROM USER_INDEXES U WHERE TABLE_NAME = ? AND INDEX_TYPE = 'NORMAL' AND NOT EXISTS (SELECT 1 FROM USER_CONSTRAINTS WHERE TABLE_NAME = U.TABLE_NAME AND INDEX_NAME = U.INDEX_NAME AND CONSTRAINT_TYPE IN ('U', 'P')) AND (upper(INDEX_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(INDEX_NAME)||UPPER(TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"'))) ORDER BY INDEX_NAME"); 
        _stmt_listindexes_noprefix = _connection.prepareStatement("SELECT INDEX_NAME, UNIQUENESS FROM USER_INDEXES U WHERE TABLE_NAME = ? AND INDEX_TYPE = 'NORMAL' AND NOT EXISTS (SELECT 1 FROM USER_CONSTRAINTS WHERE TABLE_NAME = U.TABLE_NAME AND INDEX_NAME = U.INDEX_NAME AND CONSTRAINT_TYPE IN ('U', 'P')) AND upper(INDEX_NAME) NOT LIKE 'EM_%' ORDER BY INDEX_NAME");        
        ...
        _stmt_listuniques_prefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'U' AND TABLE_NAME = ? AND (upper(CONSTRAINT_NAME) LIKE 'EM_"+_prefix+"_%' OR (upper(CONSTRAINT_NAME)||UPPER(TABLE_NAME) IN (SELECT upper(NAME1)||UPPER(NAME2) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"'))) ORDER BY CONSTRAINT_NAME");
        _stmt_listuniques_noprefix = _connection.prepareStatement("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'U' AND TABLE_NAME = ? AND upper(CONSTRAINT_NAME) NOT LIKE 'EM_%' ORDER BY CONSTRAINT_NAME");
        ...
        if (_filter.getExcludedViews().length == 0) {
            sql="SELECT VIEW_NAME, TEXT FROM USER_VIEWS";
             ...
             sql="SELECT VIEW_NAME, TEXT FROM USER_VIEWS WHERE VIEW_NAME NOT IN (" + getListObjects(_filter.getExcludedViews()) + ")";
        }
        if(_prefix!=null)
        	sql+=" AND (UPPER(VIEW_NAME) LIKE '"+_prefix+"_%' OR (upper(VIEW_NAME) IN (SELECT upper(name1) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"')))";
        ...
        if (_filter.getExcludedSequences().length == 0) {
        	...
        	sql="SELECT SEQUENCE_NAME, LAST_NUMBER, INCREMENT_BY FROM USER_SEQUENCES WHERE SEQUENCE_NAME NOT IN (" +  getListObjects(_filter.getExcludedSequences()) + ")";
        }
        if(_prefix!=null)
        {
        	if(!sql.contains("WHERE")) sql+=" WHERE 1=1";
        ...
        if (_filter.getExcludedTriggers().length == 0) {
            ...
            sql="SELECT TRIGGER_NAME, TABLE_NAME, TRIGGER_TYPE, TRIGGERING_EVENT, TRIGGER_BODY FROM USER_TRIGGERS WHERE TRIGGER_NAME NOT IN (" + getListObjects(_filter.getExcludedTriggers()) + ")";
        }
        if(_prefix!=null)
        {
        	if(!sql.contains("WHERE")) sql+=" WHERE 1=1";
        	sql+=" AND (UPPER(TRIGGER_NAME) LIKE '"+_prefix+"_%' OR (upper(TRIGGER_NAME) IN (SELECT upper(name1) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"')))";
        }
        ...
        if (_filter.getExcludedFunctions().length == 0) {
            ...
            sql="SELECT DISTINCT NAME FROM USER_SOURCE WHERE (TYPE = 'PROCEDURE' OR TYPE = 'FUNCTION') AND NAME NOT IN (" + getListObjects(_filter.getExcludedFunctions()) + ")";
        }
        if(_prefix!=null)
        	sql+=" AND (UPPER(NAME) LIKE '"+_prefix+"_%' OR (upper(NAME) IN (SELECT upper(name1) FROM AD_EXCEPTIONS WHERE AD_MODULE_ID='"+_moduleId+"')))";
