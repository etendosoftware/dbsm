...
public class Oracle8ModelReader extends JdbcModelReader {
    ...
    public Oracle8ModelReader(Platform platform) {
        ...
        try {
            _oracleIsoDatePattern = compiler
                    .compile("TO_DATE\\('([^']*)'\\, 'YYYY\\-MM\\-DD'\\)");
            _oracleIsoTimePattern = compiler
                    .compile("TO_DATE\\('([^']*)'\\, 'HH24:MI:SS'\\)");
    ...
    protected Column readColumn(DatabaseMetaDataWrapper metaData, Map values)
        ...
        if (column.getTypeCode() == Types.DECIMAL) {
            ...
            column
                    .setDefaultValue(unescape(column.getDefaultValue(), "'",
                            "''"));
        }
        return column;
    }
    ...
    protected boolean isAutoIncrement(Table table, Column column)
        ...
        String triggerName = getPlatform().getSqlBuilder().getConstraintName(
                "trg", table, column.getName(), null);
        String seqName = getPlatform().getSqlBuilder().getConstraintName("seq",
                table, column.getName(), null);
    ...
    protected Collection readIndices(DatabaseMetaDataWrapper metaData,
        ...
        try {
            ...
            while (rs.next()) {
                ...
                values.put("INDEX_TYPE", new Short(
                        DatabaseMetaData.tableIndexOther));
                values.put("NON_UNIQUE", "UNIQUE".equalsIgnoreCase(rs
                        .getString(3)) ? Boolean.FALSE : Boolean.TRUE);
