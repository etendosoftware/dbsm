...
public class SybaseBuilder extends SqlBuilder {
  ...
  protected void writeTableCreationStmtEnding(Table table, Map parameters) throws IOException {
    if (parameters != null) {
      ...
      for (Iterator it = parameters.entrySet().iterator(); it.hasNext();) {
        ...
        if (!"lock".equals(name) && !"at".equals(name) && !"external table at".equals(name)
            && !"on".equals(name)) {
    ...
    if (columnChanges != null) {
      for (Iterator changesPerColumnIt = columnChanges.entrySet().iterator(); changesPerColumnIt
        ...
        for (Iterator changeIt = changesPerColumn.iterator(); changeIt.hasNext();) {
          ((ColumnChange) changeIt.next()).apply(currentModel, getPlatform()
              .isDelimitedIdentifierModeOn());
        }
      }
    }
  ...
  /**
   * Processes the change of the default value of a column. Note that this method is only used if it
   * is the only change to that column.
   * 
   * @param currentModel
   *          The current database schema
   * @param desiredModel
  ...
  protected void processChange(Database currentModel, Database desiredModel,
    ...
    Table curTable = currentModel.findTable(change.getChangedTable().getName(), getPlatform()
        .isDelimitedIdentifierModeOn());
    Column curColumn = curTable.findColumn(change.getChangedColumn().getName(), getPlatform()
        .isDelimitedIdentifierModeOn());

    print(" DEFAULT ");
    if (isValidDefaultValue(change.getNewDefaultValue(), curColumn.getTypeCode())) {
      print(getDefaultValue(change.getNewDefaultValue(), curColumn.getTypeCode()));
    } else {
      print("NULL");
    }
    printEndOfStatement();
