...
public class MSSqlBuilder extends SqlBuilder {
    ...
    public void dropTable(Table table) throws IOException {
        ...
        println("    BEGIN");
        println("      EXEC ('ALTER TABLE '+@" + tableNameVar
                + "+' DROP CONSTRAINT '+@" + constraintNameVar + ")");
        println("      FETCH NEXT FROM refcursor INTO @" + tableNameVar + ", @"
                + constraintNameVar);
        println("    END");
    ...
    public String getDeleteSql(Table table, Map pkValues,
        ...
        return getQuotationOnStatement()
                + super.getDeleteSql(table, pkValues, genPlaceholders);
    }
    ...
    public String getInsertSql(Table table, Map columnValues,
            boolean genPlaceholders) {
        return getQuotationOnStatement()
                + super.getInsertSql(table, columnValues, genPlaceholders);
    }
    ...
    public String getUpdateSql(Table table, Map columnValues,
            boolean genPlaceholders) {
        return getQuotationOnStatement()
                + super.getUpdateSql(table, columnValues, genPlaceholders);
    }
    ...
    protected void processChanges(Database currentModel, Database desiredModel,
            List changes, CreationParameters params) throws IOException {
        if (!changes.isEmpty()) {
            writeQuotationOnStatement();
        }
        ...
        for (Iterator changeIt = changes.iterator(); changeIt.hasNext();) {
            ...
            if (change instanceof RemoveIndexChange) {
                ...
                removedForeignKeys.add(((RemoveForeignKeyChange) change)
                        .getForeignKey());
            } else if (change instanceof RemovePrimaryKeyChange) {
                removedPKs.add(((RemovePrimaryKeyChange) change)
                        .getChangedTable());
            }
        }
        ...
        for (Iterator changeIt = changes.iterator(); changeIt.hasNext();) {
            ...
            if ((change instanceof ColumnDataTypeChange)
                ...
                if (column.isPrimaryKey() && !removedPKs.contains(table)) {
                    ...
                    additionalChanges
                            .add(new RemovePrimaryKeyChange(table, pk));
                    additionalChanges.add(new AddPrimaryKeyChange(table, table
                            .getPrimaryKey(), pk));
                ...
                for (int idx = 0; idx < table.getIndexCount(); idx++) {
                    ...
                    if (index.hasColumn(column)
                        ...
                        additionalChanges.add(new RemoveIndexChange(table,
                                index));
                ...
                for (int tableIdx = 0; tableIdx < currentModel.getTableCount(); tableIdx++) {
                    ...
                    for (int fkIdx = 0; fkIdx < curTable.getForeignKeyCount(); fkIdx++) {
                        ...
                        if ((curFk.hasLocalColumn(column) || curFk
                                .hasForeignColumn(column))
                                && !removedForeignKeys.contains(curFk)) {
                            additionalChanges.add(new RemoveForeignKeyChange(
                                    curTable, curFk));
                            additionalChanges.add(new AddForeignKeyChange(
                                    curTable, curFk));
    ...
    protected void processTableStructureChanges(Database currentModel,
        ...
        for (Iterator changeIt = changes.iterator(); changeIt.hasNext();) {
            ...
            if (change instanceof RemovePrimaryKeyChange) {
                ...
                RemovePrimaryKeyChange removePkChange = new RemovePrimaryKeyChange(
                        pkChange.getChangedTable(), pkChange
        ...
        if (columnChanges != null) {
            ...
            for (Iterator changeIt = columnChanges.iterator(); changeIt
                ...
                change.apply(currentModel, getPlatform()
                        .isDelimitedIdentifierModeOn());
            }
        }
        ...
        for (Iterator changeIt = changes.iterator(); changeIt.hasNext();) {
            ...
            if (change instanceof AddPrimaryKeyChange) {
                ...
                AddPrimaryKeyChange addPkChange = new AddPrimaryKeyChange(
                        pkChange.getChangedTable(), pkChange.getNewName(),
    ...
    protected void processColumnChange(Table sourceTable, Table targetTable,
            Column sourceColumn, Column targetColumn, boolean typeChange)
            throws IOException {
        ...
        if (shallHaveDefault) {
            ...
            printIdentifier(getConstraintName("DF", sourceTable, sourceColumn
                    .getName(), null));
