...
public class PostgreSQLStandarization extends CombinedTranslation {
    ...
    public PostgreSQLStandarization() {
        ...
        append(new ReplacePatTranslation("<>\\s*[Aa][Ll][Ll]\\s*\\(\\s*[Aa][Rr][Rr][Aa][Yy]\\s*\\[(.*)\\]\\s*\\)", "NOT IN ($1)"));
        
        
        // postgres castings '::text', '::numeric', '::character varying', '::date', '::bpchar', '::timestamp', '::\"unknown\"' , ::timestamp with time zone
        append(new ReplacePatTranslation("::[A-Za-z\"]*( varying)?( with time zone)?", ""));
        
        // date truncs  date_trunk('month', now()) --> trunc(now(),'MM')
        // suports 3 levels of recursivily in parenthesis --> date_trunk('month', ...(...(...(...)...)...)...)
        append(new ReplacePatTranslation("[Dd][Aa][Tt][Ee]_[Tt][Rr][Uu][Nn][Cc]\\s*\\(\\s*'[Mm][Oo][Nn][Tt][Hh]'\\s*,\\s*([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*)\\s*\\)", "TRUNC($1, 'MM')"));
        append(new ReplacePatTranslation("[Dd][Aa][Tt][Ee]_[Tt][Rr][Uu][Nn][Cc]\\s*\\(\\s*'[Dd][Aa][Yy]'\\s*,\\s*([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*)\\s*\\)", "TRUNC($1, 'DD')"));
        append(new ReplacePatTranslation("[Dd][Aa][Tt][Ee]_[Tt][Rr][Uu][Nn][Cc]\\s*\\(\\s*'[Ww][Ee][Ee][Kk]'\\s*,\\s*([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*)\\s*\\)", "TRUNC($1, 'DY')"));
        append(new ReplacePatTranslation("[Dd][Aa][Tt][Ee]_[Tt][Rr][Uu][Nn][Cc]\\s*\\(\\s*'[Qq][Uu][Aa][Rr][Tt][Ee][Rr]'\\s*,\\s*([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*(\\([^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*\\))?[^(\\(\\))]*)\\s*\\)", "TRUNC($1, 'Q')"));
        
        // numeric cast in sums
        append(new ReplacePatTranslation("[Ss][Uu][Mm]\\s*\\(\\s*[Cc][Aa][Ss][Tt]\\s*\\((.*)\\s*[Aa][Ss]\\s*[Nn][Uu][Mm][Ee][Rr][Ii][Cc]\\)\\s*\\)", "SUM($1)"));
