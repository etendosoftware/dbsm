...
public class PostgrePLSQLStandarization extends CombinedTranslation {
    ...
    public PostgrePLSQLStandarization(Database database) {
        ...
        append(new ByLineTranslation(
                new ReplacePatTranslation(
                        "(\\s|\\t|\\(|')+[Nn][Uu][Mm][Ee][Rr][Ii][Cc](\\s|\\t|:|,|\\|')+",
                        "$1NUMBER$2")));
        append(new ReplaceStrTranslation(" NUMERIC(", " NUMBER("));
        append(new ReplaceStrTranslation(" NUMERIC)", " NUMBER)"));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(\\s|\\t)+NUMERIC(\\s|\\t)*$", "$1NUMBER$2")));
        append(new ReplaceStrTranslation(" NUMERIC;", " NUMBER;"));
        append(new ReplaceStrTranslation(" NUMERIC:", " NUMBER:"));
        append(new ReplaceStrTranslation("'NUMERIC'", "'NUMBER'"));
        ...
        append(new ByLineTranslation(
                new ReplacePatTranslation(
                        "(.*)([\\s|\\t])VARCHAR(.*) --OBTG:([Nn][Vv][Aa][Rr][Cc][Hh][Aa][Rr]2)--",
                        "$1$2$4$3")));
        append(new ByLineTranslation(
                new ReplacePatTranslation(
                        "(.*)([\\s|\\t])VARCHAR(.*) --OBTG:([Nn][Vv][Aa][Rr][Cc][Hh][Aa][Rr]--)",
                        "$1$2$4$3")));
        append(new ByLineTranslation(
                new ReplacePatTranslation(
                        "(.*)([\\s|\\t])VARCHAR(.*) --OBTG:([Vv][Aa][Rr][Cc][Hh][Aa][Rr]2)--",
                        "$1$2$4$3")));

        // TimeStamp Type
        append(new ReplaceStrTranslation(" TIMESTAMP,", " DATE,"));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(\\s|\\t)+TIMESTAMP(\\s|\\t)+", "$1DATE$2")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(\\s|\\t)+TIMESTAMP(\\s|\\t)*$", "$1DATE$2")));
        ...
        append(new ReplacePatTranslation("OPEN (.+?) FOR EXECUTE (.+?);",
                "OPEN$1FOR$2;"));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "^([^\\-]+)EXECUTE", "$1EXECUTE IMMEDIATE")));
        ...
        append(new ByLineTranslation(new ReplaceStrTranslation("AS ' DECLARE",
                "AS")));
        ...
        append(new ByLineTranslation(new ReplacePatTranslation(
                "DECLARE (.*) CURSOR (.*) FOR", "CURSOR $1 $2IS")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "^(.+?)([\\s|\\t|\\(]+?) NOT FOUND (.+?) --OBTG:(.*)--",
                "$1$2$4%NOTFOUND$3")));
        ...
        append(new ByLineTranslation(new ReplacePatTranslation(
                "RAISE EXCEPTION '%',\\s*SQLERRM\\s*;", "RAISE;")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "RAISE EXCEPTION '%',\\s*'Rollback'\\s*;", "RAISE;")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "RAISE EXCEPTION '%',\\s*'';", "RAISE;")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "RAISE EXCEPTION '%',\\s*(.*);", "RAISE $1;")));

        append(new ByLineTranslation(new ReplacePatTranslation(
                "--(.*) Exception;", "$1Exception;")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "--(.*)\\sPRAGMA EXCEPTION_INIT", "$1PRAGMA EXCEPTION_INIT")));
            ...
            do {
                ...
                if (paramsOutExist) {
                    ...
                    for (int ind = 0; ind < f.getName().length(); ind++)
                        nameRegExp += "["
                                + f.getName().substring(ind, ind + 1)
                                        .toUpperCase()
                                + f.getName().substring(ind, ind + 1)
                                        .toLowerCase() + "]";

                    String patternIn = "(?i)SELECT\\s*\\*\\s*INTO" + paramsOut
                            + "FROM\\s*(" + nameRegExp + ")\\s*\\(" + paramsIn
                            + "\\)";
                    String patternOut = "$" + (numParamsOut + 1) + "("
                            + paramPos + ")";
                    if (!outFunctions.contains(f.getName()))
                        outFunctions.add(f.getName());
                    patternsOutFunctions.add(new ByLineTranslation(
                            new ReplacePatTranslation(patternIn, patternOut)));
    ...
    public static String translateDefault(String defaultOrg) {
        String newDefault = defaultOrg.replaceAll(
                "[Tt][Oo]_[Dd][Aa][Tt][Ee]\\([Nn][Oo][Ww]\\(\\)\\)", "now()");
        return newDefault;
    }
}
