...
public class PostgrePLSQLTranslation extends CombinedTranslation {
    ...
    public PostgrePLSQLTranslation(Database database) {
        ...
        // Varchar Type
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(.*)([\\s|\\t])([Nn][Vv][Aa][Rr][Cc][Hh][Aa][Rr]2)(.*)",
                "$1$2VARCHAR$4 --OBTG:$3--")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(.*)([\\s|\\t])([Nn][Vv][Aa][Rr][Cc][Hh][Aa][Rr])(.*)",
                "$1$2VARCHAR$4 --OBTG:$3--")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(.*)([\\s|\\t])([Vv][Aa][Rr][Cc][Hh][Aa][Rr]2)(.*)",
                "$1$2VARCHAR$4 --OBTG:$3--")));
        append(new ReplaceStrTranslation(" BYTE)", ")"));
        // append(new ReplacePatTranslation("'VARCHAR2'", "'VARCHAR'"));

        // Now() function
        append(new ReplacePatTranslation("[Ss][Yy][Ss][Dd][Aa][Tt][Ee]",
                "now()"));
        /*
         * append(new ReplaceStrTranslation("SYSDATE,", "now(),")); append(new
         * ReplaceStrTranslation(" SYSDATE ", " now() ")); append(new
         * ReplaceStrTranslation("SYSDATE$", "now()")); append(new
         * ReplaceStrTranslation("(SYSDATE", "(now()")); append(new
         * ReplaceStrTranslation(" SYSDATE;", " now();")); append(new
         * ReplaceStrTranslation("=SYSDATE", "=now()")); append(new
         * ReplaceStrTranslation("<SYSDATE", "<now()"));
         */

        // TimeStamp Type
        append(new ReplaceStrTranslation(" DATE,", " TIMESTAMP,"));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(\\s|\\t)+[Dd][Aa][Tt][Ee](\\s|\\t)+", "$1TIMESTAMP$2")));
        append(new ByLineTranslation(new ReplacePatTranslation(
                "(\\s|\\t)+[Dd][Aa][Tt][Ee](\\s|\\t)*$", "$1TIMESTAMP$2")));
        ...
        append(new ReplaceStrTranslation("Not_Fully_Qualified",
                "INTERNAL_ERROR"));
        ...
        append(new ReplacePatTranslation("OPEN(.+?)FOR(.+?);",
        ...
        for (int i = 0; i < database.getFunctionCount(); i++) {
            if (database.getFunction(i).hasOutputParameters()) {
                ...
                if (database.getFunction(i).getTypeCode() == Types.NULL) {
                    append(new ReplaceStrTranslation(database.getFunction(i)
                            .getName()
                            + " *(", "PERFORM "
    ...
    private void appendFunctionWithOutputTranslation(Function func) {
        ...
        try {
            ...
            if (lastparam.getDefaultValue() != null
                    && !lastparam.getDefaultValue().equals("")) {
