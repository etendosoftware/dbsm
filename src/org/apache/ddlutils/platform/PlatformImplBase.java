    ...
    try {
      ...
      for (int idx = 0; idx < properties.length; idx++) {
        if (table.findColumn(properties[idx].getName()) != null) {
          if ((properties[idx].getName().equalsIgnoreCase("UPDATED") || properties[idx].getName()
            ...
            setStatementParameterValue(statement, sqlIndex++, properties[idx].getColumn()
                .getTypeCode(), date);
          } else if ((properties[idx].getName().equalsIgnoreCase("UPDATEDBY") || properties[idx]
              .getName().equalsIgnoreCase("CREATEDBY"))
              && dynaBean.get(properties[idx].getName()) == null) {
            setStatementParameterValue(statement, sqlIndex++, properties[idx].getColumn()
                .getTypeCode(), "0");
          } else {
  ...
  private int update(Connection connection, Database model, ColumnDataChange change)
    ...
    try {
      ...
      for (int i = 0; i < table.getColumnCount(); i++) {
        ...
        if (table.getColumn(i).getName().equalsIgnoreCase(change.getColumnname())) {
