...
public abstract class PlatformImplBase extends JdbcSupport implements Platform {
    ...
    public int evaluateBatch(Connection connection, String sql,
        ...
        try {
                ...
                try {
                    ...
                    if (continueOnError) {
                        ...
                        if (!command.contains("SCRIPT OPTIONS (FORCE = TRUE)")) {
                            _log.warn("SQL Command failed with: "
                                    + ex.getMessage());
                            _log.warn(command);
                            if (_log.isDebugEnabled()) {
                                _log.debug(ex);
            ...
            while (loops < MAX_LOOPS_OF_FORCED && !aForcedCommands.isEmpty()) {
                ...
                for (Iterator<String> it = aForcedCommands.iterator(); it
                        .hasNext();) {
                ...
                _log
                        .info("Executed " + commandCount
                                + " forced SQL command(s) with " + errors
                                + " error(s)");
            }
    ...
    public void alterTablesPostScript(Database currentModel,
        ...
        try {
            alterTablesPostScript(connection, currentModel, desiredModel,
                    continueOnError);
        } finally {
            returnConnection(connection);
        }
    ...
    public void alterTablesPostScript(Connection connection,
            Database currentModel, Database desiredModel,
            boolean continueOnError) throws DatabaseOperationException {
        String sql = null;
        ...
        try {
            ...
            getSqlBuilder().setWriter(buffer);
            getSqlBuilder().alterDatabasePostScript(currentModel, desiredModel,
                    null);
            sql = buffer.toString();
        } catch (IOException ex) {
            // won't happen because we're using a string writer
    ...
    public void alterData(Connection connection, Database model,
         ...
         * 
         * getSqlBuilder().setWriter(buffer); getSqlBuilder().alterData(model,
         * changes); sql = buffer.toString(); } catch (IOException ex) { //
         * won't happen because we're using a string writer }
         * evaluateBatch(connection, sql, true);
         */
    }
    ...
    public String getAlterTablesSql(Connection connection,
            Database desiredModel, List changes)
            throws DatabaseOperationException {
        String sql = null;
        Database currentModel = readModelFromDatabase(connection, desiredModel
                .getName());
    ...
    public List fetch(Database model, String sql, Table[] queryHints,
        ...
        try {
            ...
            for (ModelBasedResultSetIterator it = createResultSetIterator(
                    model, resultSet, queryHints); ((end < 0) || (rowIdx <= end))
                    && it.hasNext(); rowIdx++) {
                if (rowIdx >= start) {
                    result.add(it.next());
                } else {
     ...
     * @param dynaClass
     *            The type
     * @return The SQL required for querying for the id, or <code>null</code> if
     *         the database does not support this
     */
    ...
    public String getInsertSql(Database model, DynaBean dynaBean) {
        ...
        if (properties.length == 0) {
            _log.info("Cannot insert instances of type " + dynaClass
    ...
    public void upsert(Connection connection, Database model, DynaBean dynaBean)
            throws DatabaseOperationException {
        _log.debug("Attempting to update a single row " + dynaBean
                + " into table ");
        ...
        /*
         * } else if (count != 1) {
         * _log.warn("Attempted to update a single row " + dynaBean +
         * " into table " + model.getDynaClassFor(dynaBean).getTableName() +
    ...
    public void updateinsert(Connection connection, Database model,
        ...
        if (count == 0) {
            ...
            _log.warn("Attempted to update a single row " + dynaBean
                    + " into table "
                    + model.getDynaClassFor(dynaBean).getTableName()
                    + " but changed " + count + " row(s)");
        }
    }
        ...
        try {
            ...
            for (int idx = 0; idx < properties.length; idx++) {
                if (table.findColumn(properties[idx].getName()) != null) {
                    if ((properties[idx].getName().equalsIgnoreCase("UPDATED") || properties[idx]
                            .getName().equalsIgnoreCase("CREATED"))
                            && dynaBean.get(properties[idx].getName()) == null) {
                        setStatementParameterValue(statement, sqlIndex++,
                            ...
                            "UPDATEDBY") || properties[idx].getName()
                            .equalsIgnoreCase("CREATEDBY"))
                            && dynaBean.get(properties[idx].getName()) == null) {
                        setStatementParameterValue(statement, sqlIndex++,
                                properties[idx].getColumn().getTypeCode(), "0");
                    } else {
                        setObject(statement, sqlIndex++, dynaBean,
                ...
                for (int idx = 0; (results != null) && (idx < results.length); idx++) {
                    if (results[idx] < 0) {
                        ...
                        if (Jdbc3Utils.supportsJava14BatchResultCodes()) {
                            String msg = Jdbc3Utils.getBatchResultMessage(table
                                    .getName(), idx, results[idx]);
    ...
    private int privateupdate(Connection connection, Database model,
        ...
        if (primaryKeys.length == 0) {
            _log.info("Cannot update instances of type " + dynaClass
                    + " because it has no primary keys (table "
        ...
        if (nonprimaryKeys.length == 0) {
            _log.info("Cannot update instances of type " + dynaClass
                    + " because it has no values to update (table "
                    + dynaClass.getTableName() + ")");
            return 0;
        }
    ...
    public void delete(Connection connection, Database model,
            RemoveRowDALChange change) throws DatabaseOperationException {
        PreparedStatement statement = null;
        ...
        try {
            ...
            String sql = getSqlBuilder().getDeleteSql(change.getTable(),
                    pkValues, true);
    ...
    public Database loadModelFromDatabase(ExcludeFilter filter, String prefix,
        ...
        try {
            getModelLoader().setLog(_log);
            return getModelLoader().getDatabase(connection, filter, prefix,
                    loadCompleteTables, moduleId);
        } catch (SQLException ex) {
            ex.printStackTrace();
            throw new DatabaseOperationException(ex);
        } finally {
    ...
    public Database loadModelFromDatabase(ExcludeFilter filter,
        ...
        try {
            ...
            return getModelLoader()
                    .getDatabase(connection, filter, datasetName);
        } catch (SQLException ex) {
            ex.printStackTrace();
            throw new DatabaseOperationException(ex);
        } finally {
    ...
    public void deleteDataFromTable(Connection connection, Database model,
        ...
        String sql = getDeleteDataFromTableSql(model, table, sqlfilter,
                continueOnError);
        evaluateBatch(connection, sql, continueOnError);
    }
    ...
    public void deleteDataFromTable(Connection connection, Database model,
        ...
        for (int i = 0; i < tables.length; i++) {
            try {
                getSqlBuilder().writeDeleteTable(model, tables[i],
                        sqlfilters[i]);
    ...
    protected void setObject(PreparedStatement statement, int sqlIndex,
            Object value, Column column) throws SQLException {
     ...
     * directly retrieved from the result set is returned.<br/>
     ...
     * @param columnName
     *            The name of the column; can be <code>null</code> in which case
     *            the <code>columnIdx</code> will be used instead
     * @param columnIdx
    ...
    protected String getDeleteDataFromTableSql(Database model, Table table,
            boolean continueOnError) {
    ...
    protected String getDeleteDataFromTableSql(Database model, String table,
