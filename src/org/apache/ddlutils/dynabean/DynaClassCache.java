...
/**
 * Provides a cache of dyna class instances for a specific model, as well as helper methods for
...
public class DynaClassCache {
  ...
  public DynaBean createNewInstance(Table table) throws SqlDynaException {
    try {
      ...
      throw new SqlDynaException("Could not create a new dyna bean for table " + table.getName(),
          ex);
  ...
  /**
   * Creates a new dyna bean instance for the given table and copies the values from the given
   * source object. The source object can be a bean, a map or a dyna bean. This method is useful
   * when iterating through an arbitrary dyna bean result set after performing a query, then
   * creating a copy as a DynaBean which is bound to a specific table. This new DynaBean can be kept
   * around, changed and stored back into the database.
   * 
   * @param table
   *          The table to create the dyna bean for
   * @param source
   *          Either a bean, a {@link java.util.Map} or a dyna bean that will be used to populate
   *          the resultint dyna bean
   * @return A new dyna bean bound to the given table and containing all the properties from the
   *         source object
   */
  ...
  /**
   * Returns the {@link SqlDynaClass} for the given table. If the it does not exist yet, a new one
   * will be created based on the Table definition.
   * 
   * @param table
   *          The table
