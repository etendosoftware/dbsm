...
public class DynaClassCache {
    ...
    public DynaBean createNewInstance(Table table) throws SqlDynaException {
        try {
            ...
            throw new SqlDynaException(
                    "Could not create a new dyna bean for table "
                            + table.getName(), ex);
    ...
    /**
     * Creates a new dyna bean instance for the given table and copies the
     * values from the given source object. The source object can be a bean, a
     * map or a dyna bean. This method is useful when iterating through an
     * arbitrary dyna bean result set after performing a query, then creating a
     * copy as a DynaBean which is bound to a specific table. This new DynaBean
     * can be kept around, changed and stored back into the database.
     * 
     * @param table
     *            The table to create the dyna bean for
     * @param source
     *            Either a bean, a {@link java.util.Map} or a dyna bean that
     *            will be used to populate the resultint dyna bean
     * @return A new dyna bean bound to the given table and containing all the
    ...
    public SqlDynaClass getDynaClass(Table table) {
        SqlDynaClass answer = (SqlDynaClass) _dynaClassCache.get(table
                .getName());
    ...
    public SqlDynaClass getDynaClass(DynaBean dynaBean) throws SqlDynaException {
        ...
        if (dynaClass instanceof SqlDynaClass) {
            ...
            throw new SqlDynaException(
                    "The dyna bean is not an instance of a SqlDynaClass");
        }
    }

    /**
     * Creates a new {@link SqlDynaClass} instance for the given table based on
