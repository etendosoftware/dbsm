...
/**
 * Represents the addition of a foreign key to a table. Note that for simplicity and because it fits
 * the model, this change actually implements table change for the table that the new foreign key
 * will originate.
 * 
 * @version $Revision: $
 */
public class AddForeignKeyChange extends TableChangeImplBase {
  ...
  public void apply(Database database, boolean caseSensitive) {
    ...
    try {
      ...
      newFK
          .setForeignTable(database.findTable(_newForeignKey.getForeignTableName(), caseSensitive));
