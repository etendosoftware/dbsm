...
public class ModelComparator {
    ...
    public List compare(Database sourceModel, Database targetModel) {
        ...
        for (int trIdx = 0; trIdx < sourceModel.getTriggerCount(); trIdx++) {
            ...
            Trigger targetTrigger = findCorrespondingTrigger(targetModel,
                    sourceTrigger);

            if (targetTrigger == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Trigger " + sourceTrigger
        ...
        for (int trIdx = 0; trIdx < targetModel.getTriggerCount(); trIdx++) {
            ...
            Trigger sourceTrigger = findCorrespondingTrigger(sourceModel,
                    targetTrigger);

            if (sourceTrigger == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Trigger " + targetTrigger
        ...
        for (int tableIdx = 0; tableIdx < targetModel.getTableCount(); tableIdx++) {
            ...
            Table sourceTable = sourceModel.findTable(targetTable.getName(),
                    _caseSensitive);

            if (sourceTable == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Table " + targetTable.getName()
                            + " (added)");
                }
                ...
                changes.addAll(compareTables(sourceModel, sourceTable,
        ...
        for (int tableIdx = 0; tableIdx < sourceModel.getTableCount(); tableIdx++) {
            ...
            Table targetTable = targetModel.findTable(sourceTable.getName(),
                    _caseSensitive);

            if ((targetTable == null) && (sourceTable.getName() != null)
                    && (sourceTable.getName().length() > 0)) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Table " + sourceTable.getName()
                            + " (removed)");
                }
                ...
                for (int fkIdx = 0; fkIdx < sourceTable.getForeignKeyCount(); fkIdx++) {
                    changes.add(new RemoveForeignKeyChange(sourceTable,
                            sourceTable.getForeignKey(fkIdx)));
                }
            }
        }
        ...
        for (int sqIdx = 0; sqIdx < sourceModel.getSequenceCount(); sqIdx++) {
            ...
            Sequence targetSequence = findCorrespondingSequence(targetModel,
                    sourceSequence);

            if (targetSequence == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Sequence " + sourceSequence
                            + " (removed in " + sourceModel.getName() + ")");
                }
        ...
        for (int sqIdx = 0; sqIdx < targetModel.getSequenceCount(); sqIdx++) {
            ...
            Sequence sourceSequence = findCorrespondingSequence(sourceModel,
                    targetSequence);

            if (sourceSequence == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Sequence " + targetSequence
                            + " (created in " + sourceModel.getName() + ")");
                }
        ...
        for (int viIdx = 0; viIdx < sourceModel.getViewCount(); viIdx++) {
            ...
            if (targetView == null) {
                ...
                while (i < targetModel.getViewCount() && !foundViewWithSameName) {
                    if (targetModel.getView(i).getName().equalsIgnoreCase(
                            sourceView.getName()))
                ...
                {
                    if (_log.isInfoEnabled()) {
                        _log.info("Processing View " + sourceView
                                + " (removed from database "
                                + sourceModel.getName() + ")");
                    }
        ...
        for (int viIdx = 0; viIdx < targetModel.getViewCount(); viIdx++) {
            ...
            if (sourceView == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing View " + targetView
                            + " (created for the database "
                            + sourceModel.getName() + ")");
                }
        ...
        for (int fnIdx = 0; fnIdx < sourceModel.getFunctionCount(); fnIdx++) {
            ...
            if (targetFunction == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Function " + sourceFunction
                            + " (removed from database "
                            + sourceModel.getName() + ")");
                }
        ...
        for (int fnIdx = 0; fnIdx < targetModel.getFunctionCount(); fnIdx++) {
            ...
            Function sourceFunction = findCorrespondingFunction(sourceModel,
                    targetFunction);

            if (sourceFunction == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Function " + targetFunction
                            + " (created for the database "
                            + sourceModel.getName() + ")");
                }
     ...
     * @return The changes
     */
    public List compareTables(Database sourceModel, Table sourceTable,
            Database targetModel, Table targetTable) {
        ArrayList changes = new ArrayList();

        for (int fkIdx = 0; fkIdx < sourceTable.getForeignKeyCount(); fkIdx++) {
            ForeignKey sourceFk = sourceTable.getForeignKey(fkIdx);
            ForeignKey targetFk = findCorrespondingForeignKey(targetTable,
                    sourceFk);

            if (targetFk == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Foreign key " + sourceFk
                            + " (removed from table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int fkIdx = 0; fkIdx < targetTable.getForeignKeyCount(); fkIdx++) {
            ...
            ForeignKey sourceFk = findCorrespondingForeignKey(sourceTable,
                    targetFk);

            if (sourceFk == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Foreign key " + targetFk
        ...
        for (int chIdx = 0; chIdx < sourceTable.getCheckCount(); chIdx++) {
            ...
            if (targetCh == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Check " + sourceCh
                            + " (removed from table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int chIdx = 0; chIdx < targetTable.getCheckCount(); chIdx++) {
            ...
            if (sourceCh == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Check " + targetCh
        ...
        for (int uniqueIdx = 0; uniqueIdx < sourceTable.getUniqueCount(); uniqueIdx++) {
            ...
            Unique targetUnique = findCorrespondingUnique(targetTable,
                    sourceUnique);

            if (targetUnique == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Unique " + sourceUnique.getName()
                            + " (removed from table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int uniqueIdx = 0; uniqueIdx < targetTable.getUniqueCount(); uniqueIdx++) {
            ...
            Unique sourceUnique = findCorrespondingUnique(sourceTable,
                    targetUnique);

            if (sourceUnique == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Unique " + targetUnique.getName()
                            + " (created for table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int indexIdx = 0; indexIdx < sourceTable.getIndexCount(); indexIdx++) {
            ...
            if (targetIndex == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Index " + sourceIndex.getName()
                            + " (removed from table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int indexIdx = 0; indexIdx < targetTable.getIndexCount(); indexIdx++) {
            ...
            if (sourceIndex == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Index " + targetIndex.getName()
                            + " (created for table " + sourceTable.getName()
                            + ")");
                }
        ...
        for (int columnIdx = 0; columnIdx < targetTable.getColumnCount(); columnIdx++) {
            ...
            Column sourceColumn = sourceTable.findColumn(
                    targetColumn.getName(), _caseSensitive);

            if (sourceColumn == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Column " + targetColumn.getName()
                            + " (created for table " + sourceTable.getName()
                            + ")");
                }
                ...
                AddColumnChange change = new AddColumnChange(
                        sourceTable,
                        targetColumn,
                        columnIdx > 0 ? targetTable.getColumn(columnIdx - 1)
                                : null,
                        columnIdx < targetTable.getColumnCount() - 1 ? targetTable
                                .getColumn(columnIdx + 1)
                                : null);
        ...
        for (int columnIdx = targetTable.getColumnCount() - 1; columnIdx >= 0; columnIdx--) {
            Column targetColumn = targetTable.getColumn(columnIdx);
            AddColumnChange change = (AddColumnChange) addColumnChanges
                    .get(targetColumn);
        ...
        if ((sourcePK.length == 0) && (targetPK.length > 0)) {
            if (_log.isInfoEnabled()) {
                _log.info("A primary key will be added to the table "
                        + sourceTable.getName());
            }
            // we have to use the target table here because the primary key
            // might
            // reference a new column
            changes.add(new AddPrimaryKeyChange(targetTable, targetTable
                    .getPrimaryKey(), targetPK));
        } else if ((targetPK.length == 0) && (sourcePK.length > 0)) {
            if (_log.isInfoEnabled()) {
                _log.info("The primary key will be removed from the table "
                        + sourceTable.getName());
            }
            ...
            if (!UtilsCompare.equals(sourceTable.getPrimaryKey(), targetTable
                    .getPrimaryKey())) {
                changePK = true;
            } else if (sourcePK.length != targetPK.length) {
                changePK = true;
            } else {
                for (int pkColumnIdx = 0; (pkColumnIdx < sourcePK.length)
                        && !changePK; pkColumnIdx++) {
                    if ((_caseSensitive && !sourcePK[pkColumnIdx].getName()
                            .equals(targetPK[pkColumnIdx].getName()))
                            || (!_caseSensitive && !sourcePK[pkColumnIdx]
            ...
            if (changePK) {
                if (_log.isInfoEnabled()) {
                    _log.info("The primary key of table "
                            + sourceTable.getName() + " will be changed");
                }
                changes.add(new PrimaryKeyChange(sourceTable, targetTable
                        .getPrimaryKey(), sourcePK, targetPK));
            }
        }
        ...
        for (int columnIdx = 0; columnIdx < sourceTable.getColumnCount(); columnIdx++) {
            ...
            Column targetColumn = targetTable.findColumn(
                    sourceColumn.getName(), _caseSensitive);

            if (targetColumn == null) {
                if (_log.isInfoEnabled()) {
                    _log.info("Processing Column " + sourceColumn.getName()
                            + " (removed from table " + sourceTable.getName()
                            + ")");
                }
                ...
                if (targetColumnIdx != (columnIdx - diffPos)) {
                    columnPosChanges.put(sourceColumn, new Integer(
                            targetColumnIdx));
                }
            }
        }
    ...
    public List compareColumns(Table sourceTable, Column sourceColumn,
        ...
        if (_platformInfo.getComparerJDBCType(targetColumn.getTypeCode()) != _platformInfo
            ...
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the type)");
            }
        ...
        boolean scaleMatters = _platformInfo.hasPrecisionAndScale(sourceColumn
                .getTypeCode());

        if (sizeMatters
                && !StringUtils.equals(sourceColumn.getSize(), targetColumn
                        .getSize())) {

            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the size)");
            }
            changes.add(new ColumnSizeChange(sourceTable, sourceColumn,
                ...
                && (!StringUtils.equals(sourceColumn.getSize(), targetColumn
                        .getSize())
                        || (sourceColumn.getScale() == null && targetColumn
                                .getScale() != null) || (sourceColumn
                        .getScale() != null && !sourceColumn.getScale().equals(
                        targetColumn.getScale())))) {
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the scale)");
            }
            changes.add(new ColumnSizeChange(sourceTable, sourceColumn,
                    targetColumn.getSizeAsInt(), targetColumn.getScale()));
        }
        ...
        if (((sourceDefaultValue == null) && (targetDefaultValue != null))
                || ((sourceDefaultValue != null) && !sourceDefaultValue
                        .equals(targetDefaultValue))) {
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the default value)");
            }
            changes.add(new ColumnDefaultValueChange(sourceTable, sourceColumn,
        ...
        if ((sourceOnCreateDefault == null && targetOnCreateDefault != null)
                || ((sourceOnCreateDefault != null) && !sourceOnCreateDefault
                        .equals(targetOnCreateDefault))) {
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the onCreateDefault value)");
            }
            changes.add(new ColumnOnCreateDefaultValueChange(sourceTable,
                    targetColumn, targetColumn.getOnCreateDefault()));
        }
        ...
        if (sourceColumn.isRequired() != targetColumn.isRequired()) {
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the required attribute)");
            }
        ...
        if (sourceColumn.isAutoIncrement() != targetColumn.isAutoIncrement()) {
            if (_log.isInfoEnabled()) {
                _log.info("Processing Column " + sourceColumn.getName()
                        + " of table " + sourceTable.getName()
                        + " (changed because of the autoincrement attribute)");
            }
            changes
                    .add(new ColumnAutoIncrementChange(sourceTable,
                            sourceColumn));
        }
    ...
    /**
     * Searches in the given table for a corresponding unique. If the given
     * unique has no name, then a unique to the same table with the same columns
     * in the same order is searched. If the given unique has a name, then the a
     * corresponding unique also needs to have the same name, or no name at all,
     ...
     * has no name, then a check to the same table with the same columns (but
     * not necessarily in the same order) is searched. If the given check has a
     * name, then the corresponding check also needs to have the same name, or
     * no name at all, but not a different one.
     * 
     * @param table
     *            The table to search in
     * @param check
    ...
    /**
     * Searches in the given database for a corresponding sequence. If the given
     * sequence has no name, then a sequence (but not necessarily in the same
     * order) is searched. If the given sequence has a name, then the
     * corresponding sequence also needs to have the same name, or no name at
     ...
     */
    private Sequence findCorrespondingSequence(Database database,
            Sequence sequence) {
        for (int sqIdx = 0; sqIdx < database.getSequenceCount(); sqIdx++) {
            ...
            if ((_caseSensitive && sequence.equals(curSequence))
                    || (!_caseSensitive && sequence
                            .equalsIgnoreCase(curSequence))) {
                return curSequence;
            }
        }
        return null;
    ...
    /**
     * Searches in the given database for a corresponding view. If the given
     * view has no name, then a view (but not necessarily in the same order) is
     * searched. If the given view has a name, then the corresponding view also
     * needs to have the same name, or no name at all, but not a different one.
     * 
     * @param database
     *            The database to search in
     * @param view
    ...
    /**
     * Searches in the given database for a corresponding function. If the given
     * function has no name, then a function (but not necessarily in the same
     * order) is searched. If the given function has a name, then the
     * corresponding function also needs to have the same name, or no name at
     * all, but not a different one.
     * 
     * @param database
     *            The database to search in
     ...
     */
    private Function findCorrespondingFunction(Database database,
            Function function) {
        for (int fnIdx = 0; fnIdx < database.getFunctionCount(); fnIdx++) {
            ...
            if ((_caseSensitive && function.equals(curFunction))
                    || (!_caseSensitive && function
                            .equalsIgnoreCase(curFunction))) {
                return curFunction;
            }
        }
        return null;
    ...
    /**
     * Searches in the given database for a corresponding trigger. If the given
     * trigger has no name, then a trigger (but not necessarily in the same
     * order) is searched. If the given trigger has a name, then the
     * corresponding trigger also needs to have the same name, or no name at
     * all, but not a different one.
     * 
     * @param database
     *            The database to search in
