...
public class DatabaseData {
  ...
  public void changeRow(Table table, Column column, Object[] primaryKeys, Object oldValue,
    ...
    if (table == null) {
      ...
      if (column == null) {
        System.out.println("Error: impossible to change row in table " + table + ", as the column "
            + column.getName() + " doesn't exist.");
      } else {
        Vector<DynaBean> rows = getRowsFromTable(table.getName());
        int i = 0;
        boolean found = false;
        ...
        if (found) {
          ...
          if (!(oldValue == null && currentValue == null)
              && ((oldValue == null && currentValue != null)
                  || (oldValue != null && currentValue == null) || (!currentValue.equals(oldValue)))) {
